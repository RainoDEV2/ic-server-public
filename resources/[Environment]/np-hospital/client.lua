...
RegisterNetEvent("attempt:change:check-in", function()
	if exports['np_handler']:isPed("myjob") == "ems" then
...
AddEventHandler("np-hospitalization:page", function()
	if exports["np_handler"]:isPed("countems") >= 1 then
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["np-taskbar"]:taskBar(2500,"Paging a Doctor")
		if finished == 100 then
			if exports["np_handler"]:isPed("countems") >= 1 and not isTriageEnabled then
