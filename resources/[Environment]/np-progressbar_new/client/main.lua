...
function Process(action, start, tick, finish)
	...
	if Action.icon then
		if QBCore.Shared.Items[tostring(Action.icon)] then
			local img = "nui://np-inventory/nui/" -- default qb-core inventory link
			if not string.find(QBCore.Shared.Items[tostring(Action.icon)].image, "http") then -- 👀 Slipped in support for custom html links too
				if not string.find(QBCore.Shared.Items[tostring(Action.icon)].image, "icons/") then --search for if the icon images have /images in the listing
					img = img.."icons/"
				end
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent('DoLongHudText', "You're already doing something!", 2)
        end
    else
        TriggerEvent('DoLongHudText', "Cant do that action!", 2)
    end
end
