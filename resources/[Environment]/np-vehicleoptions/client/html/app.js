...
$(function () {
    ...
    $('.door').on('click', function () {
        ...
        $.post('http://np-vehicleoptions/openDoor', JSON.stringify({
            doorIndex: doorIndex
        })
        );
    });
    ...
    $('.seat').on('click', function () {
        ...
        $.post('http://np-vehicleoptions/switchSeat', JSON.stringify({
            seatIndex: seatIndex
        })
        );
    });
    ...
    $('.close').on('click', function () {
        $.post('http://np-vehicleoptions/escape', JSON.stringify({})
        );
    });

    $('.alldoor').on('click', function () {
        if (frontPanel === true) {
            $.post('http://np-vehicleoptions/togglealldoor', JSON.stringify({}));
        }
    });

    $('.lockdoors').on('click', function () {
        if (frontPanel === true) {
            $.post('http://np-vehicleoptions/lockdoors', JSON.stringify({}));
        }
    });

    $('.roof').on('click', function () {
        if (frontPanel === true) {
            $.post('http://np-vehicleoptions/convertroof', JSON.stringify({}));
        }
    });

    $('.window').on('click', function () {
        var windowIndex = $(this).attr('value');
        $.post('http://np-vehicleoptions/togglewindow', JSON.stringify({
    ...
    $('.engine').on('click', function () {
        if (frontPanel === true) {
            $.post('http://np-vehicleoptions/toggleengine', JSON.stringify({}));
        }
    });

    $('.power-key').on('click', function () {
        if (frontPanel === true) {
            $.post('http://np-vehicleoptions/givekeys', JSON.stringify({}));
        }
    });
