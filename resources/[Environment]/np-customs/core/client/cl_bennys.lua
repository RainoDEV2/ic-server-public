...
local nearHayes2 = false
local nearAutoExotics = false
local nearAutoExotics2 = false
local nearHarmony = false
local nearHarmonyBay2 = false

local bennyLocationTuner = vector3(938.37, -970.82, 39.76)
local harmonyautos = vector3(1174.39, 2640.23, 37.08)
local harmonyautos2 = vector3(1182.2590332031, 2640.8779296875, 37.14826965332)
local AutoExoticsPlace = vector3(546.57568359375, -189.33773803711, 54.493179321289)
local AutoExoticsPlace2 = vector3(548.65960693359, -198.78594970703, 53.887615203857)
local hayesautos = vector3(-1423.8599853516, -449.69671630859, 35.30366897583)
local hayesautos2 = vector3(-1417.0341796875, -446.75274658203, 35.304809570312)
...
function AttemptPurchase(type, upgradeLevel)
    ...
    if nearTuner or nearAutoExotics or nearHayes or nearHarmony or nearHarmonyBay2 or nearHayes2 or nearAutoExotics2 then
        cheap = true
    end
    ...
    TriggerServerEvent("np-customs:attemptPurchase",cheap, type, upgradeLevel)
...
RegisterNetEvent('event:control:customs')
AddEventHandler('event:control:customs', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        bennyHeading = 319.73135375977
        if useID == 1 and not isPlyInBennys and exports["np_handler"]:isPed("myJob") == "harmony_autos" then -- Bennys
            bennyHeading = 357.42
            enterLocation(harmonyautos)
        elseif useID == 5 and not isPlyInBennys and exports["np_handler"]:isPed("myJob") == "harmony_autos" then -- Bennys
            bennyHeading = 359.76287841797
            enterLocation(harmonyautos2)
        elseif useID == 2 and not isPlyInBennys and exports["np_handler"]:isPed("myJob") == "tuner_shop" then
            bennyHeading = 265.4372253418
            enterLocation(bennyLocationTuner)
        elseif useID == 3 and not isPlyInBennys and exports["np_handler"]:isPed("myJob") == "hayes_autos" then
            bennyHeading = 32.436180114746
            enterLocation(hayesautos)
        elseif useID == 6 and not isPlyInBennys and exports["np_handler"]:isPed("myJob") == "hayes_autos" then
            bennyHeading = 32.565299987793
            enterLocation(hayesautos2)
        elseif useID == 4 and not isPlyInBennys and exports["np_handler"]:isPed("myJob") == "auto_exotics" then
            bennyHeading = 91.957901000977
            enterLocation(AutoExoticsPlace)
        elseif useID == 7 and not isPlyInBennys and exports["np_handler"]:isPed("myJob") == "auto_exotics" then
            bennyHeading = 2.8877704143524
            enterLocation(AutoExoticsPlace2)
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            nearHayes2 = isNear(plyPos, hayesautos2, 5)
            nearHarmony = isNear(plyPos, harmonyautos, 5)
            nearHarmonyBay2 = isNear(plyPos, harmonyautos2, 5)
            
            nearAutoExotics = isNear(plyPos, AutoExoticsPlace, 5)
            nearAutoExotics2 = isNear(plyPos, AutoExoticsPlace2, 5)
            if nearTuner or nearHarmony or nearHarmonyBay2 or nearHayes or nearHayes2 or nearAutoExotics or nearAutoExotics2 then
                local job = exports["np_handler"]:isPed("myJob")
 
                -- Harmony Auto
                if nearHarmony and job == "harmony_autos" then
                    if not isPlyInBennys then
                        Draw3DText(harmonyautos.x, harmonyautos.y, harmonyautos.z + 0.5, "[Press ~p~E~w~ - Enter Harmony Autos]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:customs', 1)
                        end
                    else
                        disableControls()
                    end
                ...
                if nearHarmonyBay2 and job == "harmony_autos" then
                    if not isPlyInBennys then
                        Draw3DText(harmonyautos2.x, harmonyautos2.y, harmonyautos2.z + 0.5, "[Press ~p~E~w~ - Enter Harmony Autos]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:customs', 5)
                        end
                    else
                        disableControls()
                    end
                ...
                if nearAutoExotics and job == "auto_exotics" then
                    if not isPlyInBennys then
                        ...
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:customs', 4)
                        end
                    else
                        disableControls()
                    end
                ...
                if nearAutoExotics2 and job == "auto_exotics" then
                    if not isPlyInBennys then
                        Draw3DText(AutoExoticsPlace2.x, AutoExoticsPlace2.y, AutoExoticsPlace2.z + 0.5, "[Press ~p~E~w~ - Enter Auto Exotic's]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:customs', 7)
                        end
                    else
                        disableControls()
                    end
                ...
                if nearTuner and job == "tuner_shop" then
                    if not isPlyInBennys then
                        ...
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:customs', 2)
                        end
                    else
                       disableControls()
                    end
                ...
                -- Auto Bodies
                if nearHayes and job == "hayes_autos" then
                    if not isPlyInBennys then
                        ...
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:customs', 3)
                        end
                    else
                       disableControls()
                    end
                ...
                if nearHayes2 and job == "hayes_autos" then
                    if not isPlyInBennys then
                        Draw3DText(hayesautos2.x, hayesautos2.y, hayesautos2.z + 0.5, "[Press ~p~E~w~ - Enter Hayes Auto]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:customs', 6)
                        end
                    else
                       disableControls()
                    end
...
--#[Event Handlers]#--
RegisterNetEvent("np-customs:purchaseSuccessful")
AddEventHandler("np-customs:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("np-customs:purchaseFailed")
AddEventHandler("np-customs:purchaseFailed", function()
