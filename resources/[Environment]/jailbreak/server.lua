...
AddEventHandler("jail:cuttime", function()
    ...
    exports.ghmattimysql:execute("SELECT * FROM `characters` WHERE id = @cid", {['cid'] = char.id}, function(data)
        ...
        exports.ghmattimysql:execute("UPDATE `characters` SET `jail_time` = @time WHERE `id` = @cid", {['time'] = recent - tonumber(3), ['cid'] = char.id})
        user:addMoney(cash)
        TriggerClientEvent("chatMessage", src, "DOC | " , { 33, 118, 255 }, "You now have " .. recent - tonumber(random) .. " month(s) remaining")
    end)
...
RegisterCommand('unjail', function(source, args)
    ...
    local user = exports["np-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local target = exports["np-fw"]:getModule("Player"):GetUser(tonumber(args[1]))

    if user:getVar("job") == 'police' or user:getVar("job") == 'doc' then
        if tonumber(args[1]) and exports["np-fw"]:getModule("Player"):GetUser(tonumber(args[1])) then
