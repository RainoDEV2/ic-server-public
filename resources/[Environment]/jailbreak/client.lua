...
AddEventHandler("jailbreak:reclaimitems", function()
    TriggerServerEvent("server-jail-item", 'ply-'..exports["np_handler"]:isPed("cid"), false)
      ...
      while jailbreak do 
        Wait(1)
          ...
          if #(vector3(1643.5603027344,2585.4670410156,45.564853668213) - GetEntityCoords(PlayerPedId())) < 1 then
            exports["np-fw"]:getModule("Util"):MissionText("Enter Court Yard", 500)
          ...
          elseif #(vector3(1636.2059326172,2565.4235839844,45.56485748291) - GetEntityCoords(PlayerPedId())) < 1 then
            exports["np-fw"]:getModule("Util"):MissionText("Exit Court Yard", 500)
            if not teleported then
              teleported = true
...
AddEventHandler('beginJail', function(skipintake,time,name,cid,date)
    TriggerServerEvent('server-jail-item', 'ply-'..exports["np_handler"]:isPed("cid"), true)
    imjailed = false
    local playerPed = PlayerPedId()
    local mycid = exports["np_handler"]:isPed("cid")

    local gang = exports["np_handler"]:isPed("gang")
    if gang == 4 then
