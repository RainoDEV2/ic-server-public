...
AddEventHandler('phone:callContact', function(cid, targetnumber, toggle)
    ...
    if not toggle then
        ...
        for _, player in ipairs(GetPlayers()) do
            local user = exports["np-fw"]:getModule("Player"):GetUser(tonumber(player))
...
AddEventHandler('phone:StartCallConfirmed', function(mySourceID)
    ...
    TriggerClientEvent('phone:callFullyInitiated', src, mySourceID)
...
AddEventHandler('phone:EndCall', function(mySourceID, stupidcallnumberidk, somethingextra)
    ...
    if mySourceID ~= 0 or mySourceID ~= nil then
        TriggerClientEvent('phone:removefromToko', src, mySourceID, stupidcallnumberidk)
        TriggerClientEvent('phone:otherClientEndCall', src, mySourceID)
    end
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
    ...
    local user = exports["np-fw"]:getModule("Player"):GetUser(sourcePlayer)
    local char = user:getVar("character")
...
AddEventHandler('race:completed2', function(fastestLap, overall, sprint, identifier)
    ...
    local cid = char.id
    local playername = ""..char.first_name.." "..char.last_name..""
    print('Char ', json.encode(char))
    print('Player Name ', playername)
    print('ID = ', json.encode(Races[identifier].map))
    print(json.encode(Races[identifier], {indent = true}))
...
--     local src = source
--     local user = exports["np-fw"]:getModule("Player"):GetUser(tonumber(id))
--     local char = user:getCurrentCharacter()
    ...
    exports.ghmattimysql:execute("SELECT * FROM group_banking WHERE group_type = @id", {['id'] = groupname}, function(data)
        if data[1].bank >= tonumber(amount) then
            exports.ghmattimysql:execute("SELECT paycheck FROM characters WHERE id = @id", {['id'] = cid}, function(result)
                ...
                local pLogData = "Sender Name: " ..GetPlayerName(src).. "  | CID: " ..cid.. " Received $" .. amount.. " | ["..groupname.."]"
                exports['np-fw']:k_log(src, "external-pay", pLogData)
            end)
        else
...
AddEventHandler('server:gankGroup', function(groupid, amount)
    ...
    if user:getCash() >= tonumber(amount) then
        ...
        exports.ghmattimysql:execute("SELECT * FROM group_banking WHERE group_type = @id", {['id'] = groupid}, function(result)
            ...
            local pLogData = GetPlayerName(src).. " Deposited $" .. amount.. " into ["..groupid.."] account"
            exports['np-fw']:k_log(src, "external-deposit", pLogData)
        end)
    end
end)
