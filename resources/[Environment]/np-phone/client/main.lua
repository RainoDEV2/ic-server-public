...
RegisterNUICallback('btnProperty2', function(data, cb)
  loading()
  TriggerServerEvent("ReturnHouseKeys", exports['np_handler']:isPed('cid'))
end)
...
function GroupName(groupid)
  ...
  local mypasses = exports["np_handler"]:isPed("passes")
  for i=1, #mypasses do
...
AddEventHandler("phone:Garage", function(plyvehs)
  ...
  local rankCarshop = exports["np_handler"]:GroupRank("car_shop")
...
function hasDecrypt2()
  if exports["np-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["np_handler"]:isPed("disabled") then
    return true
  else
    return false
  end
...
function hasDevice()
    if exports["np-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["np_handler"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      ...
      and not exports["np_handler"]:isPed("disabled") and not exports["np_handler"]:isPed("handcuffed")
      ) 
    then
      return true
    else
...
function hasRadio()
    if exports["np-inventory"]:hasEnoughOfItem("radio",1,false) and not exports["np_handler"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    closeGui()
    if not exports["np_handler"]:isPed("disabled") then
...
function miTrabajo()
    return exports['np_handler']:isPed('job')
end
...
function AddCheckPoint()
  ...
  local fx,fy,fz = table.unpack(GetOffsetFromEntityInWorldCoords(PlayerPedId(),  ballsdick, 0.0, -0.25))

  local fx2,fy2,fz2 = table.unpack(GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0 - ballsdick, 0.0, -0.25))
...
RegisterNUICallback('racing:events:list', function()
  local rank = exports["np_handler"]:GroupRank("ug_racing")
  if exports["np-inventory"]:hasEnoughOfItem("racingusb1",1,false) then
  SendNUIMessage({
...
RegisterNUICallback('racing:event:join', function(data)
  ...
  local ped = PlayerPedId()
  local IsPlayerDriver = GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId()
  local plyCoords = GetEntityCoords(ped)
...
RegisterNUICallback('messages', function(data, cb)
  loading()
  TriggerServerEvent('phone:getSMS', exports['np_handler']:isPed('cid'))
  cb('ok')
end)
...
AddEventHandler('refreshContacts', function()
  TriggerServerEvent('getContacts', exports['np_handler']:isPed('cid'))
...
AddEventHandler('phone:makecall', function(pnumber)
  ...
  if callStatus == isNotInCall and not isDead and hasPhone() then
    ...
    TriggerServerEvent('phone:callContact', exports['np_handler']:isPed('cid'), pnumber, true)
  else
...
AddEventHandler('phone:makepayphonecall', function(pnumber) 
    ...
    if callStatus == isNotInCall and not isDead then
      ...
      TriggerServerEvent('phone:callContact', exports['np_handler']:isPed('cid'), pnumber, false)
...
RegisterNUICallback('callContact', function(data, cb)
  ...
  if callStatus == isNotInCall and not isDead and hasPhone() then
    ...
    TriggerEvent("animation:phonecallstart")
    TriggerServerEvent('phone:callContact', exports['np_handler']:isPed('cid'), data.number, true)
  else
    TriggerEvent("DoLongHudText","It appears you are already in a call, injured or with out a phone, please type /hangup to reset your calls.",2)
  end
  cb('ok')
...
AddEventHandler('Client:UpdateTweet', function(tweet)
    ...
    local handle = exports["np_handler"]:isPed("twitterhandle")
...
RegisterNUICallback('accountInformation', function()
  TriggerServerEvent("account:information:sv", exports['np_handler']:isPed('cid'))
end)
...
AddEventHandler("account:information:cl", function(licences, paycheck, pChips)
  local responseObject = {
    cid = exports["np_handler"]:isPed("cid"),
    paycheck = paycheck,
    pChips = pChips,
    cash = exports["np_handler"]:isPed("mycash"),
    bank = exports["np_handler"]:isPed("mybank"),
    job = exports["np_handler"]:isPed("myjob"),
    secondaryJob = exports["np_handler"]:isPed("secondaryjob"),
    licenses = licences, 
    pagerStatus = exports["np_handler"]:isPed("pagerstatus")
  }
...
AddEventHandler('phone:sendSMS', function(number, message)
  if(number ~= nil and message ~= nil) then
    TriggerServerEvent('phone:sendSMS', exports['np_handler']:isPed('cid'), number, message)
    Citizen.Wait(1000)
    TriggerServerEvent('phone:getSMSc', exports['np_handler']:isPed('cid'))
  else
...
AddEventHandler('phone:reply', function(message)
  if lastnumber ~= 0 then
    TriggerServerEvent('phone:sendSMS', exports['np_handler']:isPed('cid'), lastnumber, message)
...
RegisterNUICallback('settings', function()
  local controls = exports["np-fw"]:getModule("DataControls"):getBindTable()
  local settings = exports["np-fw"]:getModule("SettingsData"):getSettingsTable()
...
RegisterNUICallback('settingsUpdateToko', function(data, cb)
  if data.tag == "settings" then
    exports["np-fw"]:getModule("SettingsData"):setSettingsTableGlobal(data.settings, true)
  elseif data.tag == "controlUpdate" then
    exports["np-fw"]:getModule("DataControls"):encodeSetBindTable(data.controls)
  end
end)

RegisterNUICallback('settingsResetToko', function()
  TriggerEvent("np-fw:cl:player_reset", "tokovoip")
end)

RegisterNUICallback('settingsResetControls', function()
  TriggerEvent("np-fw:cl:player_control", nil)
end)
...
RegisterNUICallback('PostNewImage', function(data, cb)
  ...
  while takePhoto do
    ...
    elseif IsControlJustPressed(1, 176) then
      exports['screenshot-basic']:requestScreenshotUpload('https://discord.com/api/webhooks/891010361237569577/TZ6Zc4IOwcg_5M9qHr21f0GubS7S2TuNN5jNdNRU9C5mJllSJo_tnk4RULNpc2abnvdK', 'files[]', function(data2)
