...
RegisterServerEvent("np-clothingmenu:insert_character_current")
AddEventHandler("np-clothingmenu:insert_character_current",function(data)
...
RegisterServerEvent("np-clothingmenu:insert_character_face")
AddEventHandler("np-clothingmenu:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("np-clothingmenu:get_character_face")
AddEventHandler("np-clothingmenu:get_character_face",function(pSrc)
    ...
    exports.ghmattimysql:execute("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM character_face cf INNER JOIN character_current cc on cc.cid = cf.cid WHERE cf.cid = @cid", {['cid'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("np-clothingmenu:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("np-clothingmenu:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("np-clothingmenu:get_character_current")
AddEventHandler("np-clothingmenu:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = exports["np-fw"]:getModule("Player"):GetUser(src)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_current WHERE cid = @cid", {['cid'] = cid}, function(result)
        ...
        TriggerClientEvent("np-clothingmenu:setclothes", src, temp_data, 0)
	end)
end)

RegisterServerEvent("np-clothingmenu:retrieve_tats")
AddEventHandler("np-clothingmenu:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
	local user = exports["np-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
	exports.ghmattimysql:execute("SELECT * FROM playersTattoos WHERE identifier = @identifier", {['identifier'] = char.id}, function(result)
        if(#result == 1) then
			TriggerClientEvent("np-clothingmenu:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			...
			TriggerClientEvent("np-clothingmenu:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("np-clothingmenu:set_tats")
AddEventHandler("np-clothingmenu:set_tats", function(tattoosList)
	local src = source
	local user = exports["np-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterServerEvent("np-clothingmenu:get_outfit")
AddEventHandler("np-clothingmenu:get_outfit",function(slot)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("np-clothingmenu:setclothes", src, data , 0)
...
RegisterServerEvent("np-clothingmenu:set_outfit")
AddEventHandler("np-clothingmenu:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("np-clothingmenu:remove_outfit")
AddEventHandler("np-clothingmenu:remove_outfit",function(slot)
...
RegisterServerEvent("np-clothingmenu:list_outfits")
AddEventHandler("np-clothingmenu:list_outfits",function()
    local src = source
    local user = exports["np-fw"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local slot = slot
            ...
            TriggerClientEvent('np-context:sendMenu', src, {
                {
                    ...
                    params = {
                        event = "np-clothingmenu:list:outfits",
                        args = {
...
RegisterServerEvent("np-clothingmenu:attempt:change", function(pSlot, pNewName)
    local pSrc = source
    ...
    Citizen.Wait(100)
    TriggerClientEvent("np-clothingmenu:return", pSrc)

end)

RegisterServerEvent("np-clothingmenu:options:oufits", function(pSlot)
    local src = source
    TriggerClientEvent('np-context:sendMenu', src, {
		{
			id = 1,
		...
		},
		{
			...
			params = {
				event = "np-clothingmenu:use:outfit",
				args = {
					slot = pSlot
				}
			}
		...
		},
        {
			...
			params = {
				event = "np-clothingmenu:remove:outfit",
				args = {
					slot = pSlot
				}
			}
		...
		},
        {
			...
			params = {
				event = "np-clothingmenu:attempt:change",
				args = {
					slot = pSlot
				}
			}
    ...
    exports.ghmattimysql:execute("SELECT count(rank) whitelist FROM jobs_whitelist WHERE cid = @cid LIMIT 1", {
        ...
        exports.ghmattimysql:scalar("SELECT count(model) FROM character_current WHERE cid = @cid LIMIT 1", {
            ...
            if result == 0 then
                ...
                TriggerEvent("np-clothingmenu:get_character_current", src)
            end
            TriggerClientEvent("np-clothingmenu:inService",src,isService)
    	end)
    end)
end)
...
AddEventHandler("clothing:checkMoney", function(askingPrice)
    local src = source
    local target = exports["np-fw"]:getModule("Player"):GetUser(src)
