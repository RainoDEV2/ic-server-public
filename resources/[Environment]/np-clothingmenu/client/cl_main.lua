...
local inSpawn = false
AddEventHandler("np-clothingmenu:inSpawn", function(pInSpawn)
    inSpawn = pInSpawn
end)
...
RegisterNUICallback('escape', function(data, cb)
    ...
    if shouldSave and currentPrice > 0 then
        ...
        if exports["np_handler"]:isPed("mycash") < currentPrice then
            shouldSave = false
        end
    end
    if not startingMenu then
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            TriggerServerEvent("np-clothingmenu:insert_character_current", data)
            TriggerServerEvent("np-clothingmenu:insert_character_face", data)
            TriggerServerEvent("np-clothingmenu:set_tats", currentTats)
...
AddEventHandler("np-fw:initialSpawnModelLoaded", function()
    TriggerServerEvent("clothing:checkIfNew")
end)

RegisterNetEvent("np-clothingmenu:inService")
AddEventHandler("np-clothingmenu:inService", function(service)
    isService = service
end)

RegisterNetEvent("np-clothingmenu:hasEnough")
AddEventHandler("np-clothingmenu:hasEnough", function(menu)
    if menu == "tattoo_shop" then
        TriggerServerEvent("np-clothingmenu:retrieve_tats")
...
RegisterNetEvent("np-clothingmenu:setclothes")
AddEventHandler("np-clothingmenu:setclothes", function(data,alreadyExist)
    player = PlayerPedId()
    ...
    Citizen.Wait(500)
	TriggerEvent("facewear:update")
    TriggerServerEvent("np-clothingmenu:get_character_face")
    TriggerServerEvent("np-clothingmenu:retrieve_tats")
    TriggerServerEvent("police:getAnimData")
    TriggerServerEvent("police:SetMeta")
...
RegisterNetEvent("np-clothingmenu:AdminSetModel")
AddEventHandler("np-clothingmenu:AdminSetModel", function(model)
...
RegisterNetEvent("np-clothingmenu:defaultReset")
AddEventHandler("np-clothingmenu:defaultReset", function()
    DoScreenFadeOut(10)
    ...
    TriggerEvent("np-clothingmenu:openClothing")
    TriggerEvent("np-clothingmenu:inSpawn", true)
end)

RegisterNetEvent("np-clothingmenu:settattoos")
AddEventHandler("np-clothingmenu:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("np-clothingmenu:setpedfeatures")
AddEventHandler("np-clothingmenu:setpedfeatures", function(data)
    player = PlayerPedId()
    if data == false then
...
RegisterNetEvent('np-clothingmenu:openClothing')
AddEventHandler('np-clothingmenu:openClothing', function(pDontShowBarber, pShouldCost)
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        TriggerEvent('np-textui:ShowUI', 'show', ("[M] %s"):format(currentZone.text))
...
RegisterNetEvent('np-clothingmenu:outfits')
AddEventHandler('np-clothingmenu:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("np-clothingmenu:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("np-clothingmenu:remove_outfit",pId)
    elseif pAction == 3 then 
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("np-clothingmenu:get_outfit", pId)
    else
...
RegisterCommand("m1", function(source, args, rawCommand)
    if exports["np-inventory"]:hasEnoughOfItem("mask",1) then
        ...
        TriggerEvent("DoLongHudText", "You don't have a mask", 2)
    end 
end, false)
...
RegisterNetEvent('np-clothingmenu:enable')
AddEventHandler('np-clothingmenu:enable', function(status)
    actionDress = status
end)
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] and args[2] then
            TriggerEvent('np-clothingmenu:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if actionDress == true then
        TriggerEvent('np-clothingmenu:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  
...
RegisterNetEvent("np-clothingmenu:list:outfits")
AddEventHandler("np-clothingmenu:list:outfits", function(data)
    TriggerServerEvent("np-clothingmenu:options:oufits", data.slot)
end)


RegisterNetEvent("np-clothingmenu:use:outfit")
AddEventHandler("np-clothingmenu:use:outfit", function(data)
    TriggerServerEvent("np-clothingmenu:get_outfit", data.slot)
end)

RegisterNetEvent("np-clothingmenu:remove:outfit")
AddEventHandler("np-clothingmenu:remove:outfit", function(data)
    TriggerServerEvent("np-clothingmenu:remove_outfit", data.slot)
end)


RegisterNetEvent("np-clothingmenu:return", function()
    TriggerEvent("hud:saveCurrentMeta")
    TriggerServerEvent("np-clothingmenu:list_outfits")
end)

RegisterNetEvent("np-clothingmenu:attempt:change", function(data)
    local pNewName = exports["np-keyboard"]:KeyboardInput({
    ...
    if pNewName then
        TriggerServerEvent('np-clothingmenu:attempt:change',  data.slot, pNewName[1].input)
    else
...
AddEventHandler("facewear:adjust",function(faceType,remove)
	local handcuffed = exports["np_handler"]:isPed("handcuffed")
