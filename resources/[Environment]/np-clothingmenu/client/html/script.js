...
$(function () {
    ...
    function CloseMenu(save) {
        $.post('https://np-clothingmenu/escape', JSON.stringify({save:save}));
    }

    $(document).on('contextmenu', function() {
        $.post('https://np-clothingmenu/togglecursor', JSON.stringify({}));
    })
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('https://np-clothingmenu/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothing_shop')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if ($(ele).parents('.panel').attr('id') == "skins") {
                $.post('https://np-clothingmenu/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('https://np-clothingmenu/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('https://np-clothingmenu/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothingmenu/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothingmenu/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothingmenu/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('https://np-clothingmenu/switchcam', JSON.stringify({name: 'cam'}))
    })

    $('.backup_save').on('click', function() {
        $.post('https://np-clothingmenu/getPlayerFace')
    })
    ...
    $('.backup_load').on('click', function() {
        $.post('https://np-clothingmenu/setPlayerFace',$("#backupJsonInput").val())
    })

    $('.tog_hat').on('click', function() {
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('https://np-clothingmenu/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('https://np-clothingmenu/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('https://np-clothingmenu/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('https://np-clothingmenu/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('https://np-clothingmenu/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('https://np-clothingmenu/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('https://np-clothingmenu/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('https://np-clothingmenu/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
