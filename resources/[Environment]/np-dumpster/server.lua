local dumpsterItems = {
    ...
    [4] = {chance = 4, id = 'bakingsoda', quantity = math.random(1,3)},
    [5] = {chance = 3, id = 'chloroform', quantity = math.random(1,3)},
    [6] = {chance = 5, id = 'dye', quantity = math.random(1,3)},
}

RegisterServerEvent('iciest :startDumpsterTimer')
AddEventHandler('iciest :startDumpsterTimer', function(dumpster)
    startTimer(source, dumpster)
end)

RegisterServerEvent('iciest :giveDumpsterReward')
AddEventHandler('iciest :giveDumpsterReward', function()
...
function startTimer(id, object)
    ...
    while timer > 0 do
        ...
        timer = timer - 1000
        if timer == 0 then
            TriggerClientEvent('iciest :removeDumpster', id, object)
        end
    end
end