...
function IRP.DB.CreateNewPlayer(self, src, callback)
    local hexid = IRP.Util:GetHexId(src)
    callback = callback and callback or function() return end

    local data = {
        hexid = hexid,
        communityid = IRP.Util:HexIdToComId(hexid),
        steamid = IRP.Util:HexIdToSteamId(hexid),
        license = IRP.Util:GetLicense(src),
...
function IRP.DB.CreateNewCharacter(self, src, data2, hexid, ph, callback)
    local hexid = hexid
    callback = callback and callback or function() return end
...
function IRP.DB.PlayerExistsDB(self, src, callback)
    local hexId = IRP.Util:GetHexId(src)
    callback = callback and callback or function() return end
...
function IRP.DB.PhoneNumberExists(self, src, phone_number, callback)
...
function IRP.DB.FetchPlayerData(self, src, callback)
    local hexId = IRP.Util:GetHexId(src)
    callback = callback and callback or function() return end
...
function IRP.DB.FetchCharacterData(self, user, callback)
    callback = callback and callback or function() return end
...
function IRP.DB.DeleteCharacter(self, user, id, callback)
    callback = callback and callback or function() return end
...
function IRP.DB.UpdateCharacterStressLevel(self, user, characterId, newLevel, callback)
    callback = callback and callback or function() return end
    if not user then callback(false,true) return end
...
function IRP.DB.UpdateCharacterDirtyMoney(self, user, characterId, amount, callback)
callback = callback and callback or function() return end
    if not user then callback(false,true) return end
    if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function IRP.DB.UpdateCharacterCasino(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function IRP.DB.UpdateCharacterMoney(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function IRP.DB.UpdateCharacterBank(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function IRP.DB.UpdateControls(self, src, controlsTable, callback)
    callback = callback and callback or function() return end
        local user = exports["np-fw"]:getModule("Player"):GetUser(src)
        if not user then callback(false,true) return end
...
function IRP.DB.GetControls(self, src, callback)
    callback = callback and callback or function() return end
    local user = exports["np-fw"]:getModule("Player"):GetUser(src)
        Citizen.Wait(3000)
        if not user then callback(false, true) return end
...
function IRP.DB.UpdateSettings(self, src, settingsTable, callback)
    callback = callback and callback or function() return end
    local user = exports["np-fw"]:getModule("Player"):GetUser(src)
        if not user then callback(false,true) return end
...
function IRP.DB.GetSettings(self, src, callback)
    callback = callback and callback or function() return end
    local user = exports["np-fw"]:getModule("Player"):GetUser(src)
    if not user then callback(false, true) return end
