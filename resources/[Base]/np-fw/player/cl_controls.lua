...
IRP.Controls.secondaryBinds = {}

IRP.Controls.Distcheck = {}
IRP.Controls.Distcheck.use = {
	-- create fake id
	...
	-- sandypd evidence
	[71] =  { ['pos'] = {1857.5465087891, 3690.7575683594, 29.818536758423}, ["r"] = 2,["event"] = "police",["id"] = 2,["maxwait"] = 300},
	-- sandypd trash
	[72] =  { ['pos'] = {0,0,0}, ["r"] = 2,["event"] = "police",["id"] = 4,["maxwait"] = 300},
	-- paleto trash
	...
	[75] ={ ["pos"] = {343.68,-597.65,43.29},	["r"] = 1,["event"] = "login",["id"] = 4,["maxwait"] = 200},
		...
		[79] ={ ["pos"] = {1853.4416503906, 3688.4875488281, 29.818534851074},	["r"] = 1,["event"] = "openpersonalpd",["id"] = 2,["maxwait"] = 200},	
...
IRP.Controls.Distcheck.useSecondary = {
...
IRP.Controls.Distcheck.useThird = {
	-- hospital garage
...
function IRP.DataControls.encodeSetBindTable(self, bindTable)
	...
	IRP.DataControls.setBindTable(controlTable,true)
end

function IRP.DataControls.toUpper(table)
...
function IRP.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		IRP.Controls.Current  = IRP.DataControls.toUpper(IRP.Controls.Default) 
		IRP.DataControls.setSecondaryBindTable(IRP.Controls.Current)
		TriggerServerEvent('np-fw:sv:player_control_set',IRP.Controls.Current)
		IRP.DataControls.checkForMissing()
	else
		if shouldSend then 
			IRP.Controls.Current = IRP.DataControls.toUpper(controlTable)
			IRP.DataControls.setSecondaryBindTable(IRP.Controls.Current)
			TriggerServerEvent('np-fw:sv:player_control_set',IRP.Controls.Current)
			IRP.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",IRP.DataControls.getTableInKeyNumbers())
end

function IRP.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#IRP.Controls.secondaryBinds  do
				if IRP.Controls.secondaryBinds[i] and IRP.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				IRP.Controls.secondaryBinds[i] = {}
				IRP.Controls.secondaryBinds[i][1] = control1
				IRP.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function IRP.DataControls.getTableInKeyNumbers()
...
function IRP.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(IRP.Controls.Current) do
		if k == bindID then
			found = v
...
function IRP.DataControls.distanceCall(distcheckName)
	local distanceTable = IRP.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if IRP.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-IRP.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			IRP.Controls.EventHolder[key] = GetGameTimer();
...
function IRP.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(IRP.Controls.Default) do
		if IRP.Controls.Current[j] == nil then
			isMissing = true
			IRP.Controls.Current[j] = h
		end
	end
...
function IRP.DataControls.validEvent(event,id,eventID)
	local v = IRP.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if IRP.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-IRP.Controls.EventHolder[eventKey])
	end

	if IRP.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			IRP.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if IRP.Controls.Current["tokoptt"] then
			for i=0,#IRP.Controls.events["general"] do
				local v = IRP.Controls.events["general"][i]
				local keyString = IRP.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#IRP.Controls.secondaryBinds do
							if IsControlPressed(1,IRP.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,IRP.Controls.secondaryBinds[i][1]) then
								isConflicted = IRP.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								IRP.DataControls.validEvent(eventBind,idEvent,i)
							else
								IRP.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#IRP.Controls.secondaryBinds do
									if IsControlPressed(1,IRP.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,IRP.Controls.secondaryBinds[i][2]) then
										isConflicted = IRP.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if IRP.Controls.Current["tokoptt"] then
			for i=0,#IRP.Controls.events["general-dist"] do
				local v = IRP.Controls.events["general-dist"][i]
				local key = Keys[IRP.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					IRP.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("np-fw:cl:player_control")
AddEventHandler("np-fw:cl:player_control", function(controlTable)
	IRP.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('np-fw:setcontrols')
AddEventHandler('np-fw:setcontrols', function()
	TriggerServerEvent('np-fw:sv:player_controls')
end)
