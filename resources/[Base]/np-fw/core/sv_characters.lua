function IRP.Core.LoginPlayer(self, args, src, callback)
    TriggerEvent("np-fw:playerAttemptLogin", src)

    local user = IRP.Player:CreatePlayer(src, false)

    if not user then
        user = IRP.Player:CreatePlayer(src, false)
    ...
    local function fetchData(_err)
        ...
        IRP.DB:FetchPlayerData(src, function(data, err)
            if err then
                data = {
                    err = true,
            ...
            if not err then TriggerEvent("np-fw:playerLoggedIn", user) TriggerClientEvent("np-fw:playerLoggedIn", src) end
        end)
    end


	IRP.DB:PlayerExistsDB(src, function(exists, err)
		if err then
		...
		if not exists then
			IRP.DB:CreateNewPlayer(src, function(created)
				if not created then
...
IRP.Events:AddEvent(IRP.Core, IRP.Core.LoginPlayer, "np-fw:loginPlayer")

function IRP.Core.FetchPlayerCharacters(self, args, src, callback)
	local user = IRP.Player:GetUser(src)

	if not user then return end

	IRP.DB:FetchCharacterData(user, function(data, err)
		if err then
			data = {
				err = true,
			...
			TriggerEvent("np-fw:charactersLoaded", user, data)
			TriggerClientEvent("np-fw:charactersLoaded", src, data)
		end

		callback(data)
	end)
end
IRP.Events:AddEvent(IRP.Core, IRP.Core.FetchPlayerCharacters, "np-fw:fetchPlayerCharacters")

function IRP.Core.CreatePhoneNumber(self, src, callback)
	Citizen.CreateThread(function()
		while true do 
			...
			if phoneNumber then 
				...
				if phoneNumber then
					IRP.DB:PhoneNumberExists(src, phoneNumber, function(exists, err)
...
function IRP.Core.CreateCharacter(self, charData, src, callback)
	local user = IRP.Player:GetUser(src)
	...
	["ln"] = ln
		...
		if result[1] ~= nil then 
			...
			self:CreatePhoneNumber(src, function(phoneNumber, err)
				...
				IRP.DB:CreateNewCharacter(user, charData, hexId, phoneNumber, function(created, err)
...
IRP.Events:AddEvent(IRP.Core, IRP.Core.CreateCharacter, "np-fw:createCharacter")

function IRP.Core.DeleteCharacter(self, id, src, callback)
	local user = IRP.Player:GetUser(src)
	...
	IRP.DB:DeleteCharacter(user, id, function(deleted)
		callback(deleted)
	end)
end
IRP.Events:AddEvent(IRP.Core, IRP.Core.DeleteCharacter, "np-fw:deleteCharacter")

function IRP.Core.SelectCharacter(self, id, src, callback)
	local user = IRP.Player:GetUser(src)
	...
	TriggerClientEvent('np-fw:setcontrols', src)
	TriggerClientEvent('updatepasses', src)
	TriggerEvent("np-fw:characterLoaded", user, selectedCharacter)
	TriggerClientEvent("np-fw:characterLoaded", src, selectedCharacter)

	callback({loggedin = true, chardata = selectedCharacter})
end
IRP.Events:AddEvent(IRP.Core, IRP.Core.SelectCharacter, "np-fw:selectCharacter")