IRP.BlipManager = IRP.BlipManager or {}
IRP.Blips = IRP.Blips or {}

function IRP.BlipManager.CreateBlip(self, id, data)
    ...
    IRP.Blips[id] = {blip = blip, data = data}
end

function IRP.BlipManager.RemoveBlip(self, id)
    local blip = IRP.Blips[id]
    if blip then RemoveBlip(blip.blip) end
    IRP.Blips[id] = nil
end

function IRP.BlipManager.HideBlip(self, id, toggle)
    local blip = IRP.Blips[id]
    if not blip then return end
    if toggle then SetBlipAlpha(blip, 0) else SetBlipAlpha(blip, 255) end
end

function IRP.BlipManager.GetBlip(self, id)
    local blip = IRP.Blips[id]
