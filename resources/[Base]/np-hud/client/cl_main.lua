...
RegisterNetEvent('np-hud:getPercentages')
AddEventHandler('np-hud:getPercentages', function()
	local cid = exports["np_handler"]:isPed("cid")
	local settings = GetResourceKvpString(cid .. '-hud')
	print(settings)
	if settings ~= nil then
		...
		local tempTable = {
			[1] = {
				name = 'health',
				percentage = decoded.health
			},
			[2] = {
				name = 'hunger',
				percentage = decoded.hunger},
			[3] = {
				name = 'armor',
				percentage = decoded.armor
			},
			[4] = {
				name = 'water',
				percentage = decoded.water
			}
		}
		for i = 1, #tempTable do
			print(tempTable[i].name .. "=" .. tempTable[i].percentage)
			SendNUIMessage({action = "update_percentage", percentageName = tempTable[i].name, percentage = tonumber(tempTable[i].percentage)})
		end
	else
		return
	end
 ...
 RegisterCommand('showmenu', function(source, args)
	SetNuiFocus(true, true)
	SendNUIMessage({action = "openPercent"})
 end)

 RegisterNUICallback("savePercentage", function(data, cb)
	local cid = exports["np_handler"]:isPed("cid")
	print(json.encode(data))
	-- print(json.encode(data.table))
	SetResourceKvp(cid .. '-hud', json.encode(data))
	TriggerEvent('DoShortHudText', 'You have saved your percentages', 1)
    cb(true)
end)

RegisterNUICallback("exit", function(data, cb)
	SetNuiFocus(false, false)
	TriggerEvent('DoShortHudText', 'You exited the menu.', 1)
    cb(true)
end)

RegisterNetEvent("np-hud:EnableHud")
AddEventHandler("np-hud:EnableHud", function()
    isLoggedIn = true
end)
...
local DevMode = false
RegisterNetEvent("np-adminmenu:currentDevmode", function(pOn)
	DevMode = pOn
end)

local DebugMode = false
RegisterNetEvent("np-adminmenu:currentDebug", function(pOn)
	DebugMode = pOn
end)
...
exports["np-keymapping"]:registerKeyMapping("Handsup Keybind", "Player", "Toggle Handsup", "+handsup", "-handsup", "", true)
...
AddEventHandler("fire:damageUser", function(Reqeuester)
	...
	if IsPedShooting(Attackerped) then
        ...
        if name == `WEAPON_FIREEXTINGUISHER` and not exports["np_handler"]:isPed("dead") then
		...
		if not IsPedSwimmingUnderWater( PlayerPedId() ) and currentValues["oxy"] < 25.0 then
			if GetGameTimer() - lastDamageTrigger > 3000 then
				...
				if currentValues["oxy"] <= 0 then
					...
					if exports["np_handler"]:isPed("dead") then
...
function attachProp2(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["np_handler"]:GlobalObject(attachedProp2)
...
function attachProp(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["np_handler"]:GlobalObject(attachedProp)
...
RegisterNetEvent("np-adminmenu:currentDebug")
AddEventHandler("np-adminmenu:currentDebug", function(debugToggle)
