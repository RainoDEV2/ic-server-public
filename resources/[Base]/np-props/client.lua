...
AddEventHandler('attachPropPerm', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["np_handler"]:GlobalObject(attachedPropPerm)
...
AddEventHandler('attachPropPhone', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["np_handler"]:GlobalObject(attachedPropPhone)
...
AddEventHandler('attachProp69', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["np_handler"]:GlobalObject(attachedProp69)
...
Citizen.CreateThread(function()
	while true do
		...
		if carryingObject then
			...
			if IsEntityPlayingAnim(PlayerPedId(), "anim@heists@box_carry@", "idle", 3) and holding == "stolenart" then
				if IsPedRunning(PlayerPedId()) or IsPedSprinting(PlayerPedId()) then
					...
					if ticks >= 600 then
						...
						TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 1.0, 'robberyglassbreak', 0.5)
