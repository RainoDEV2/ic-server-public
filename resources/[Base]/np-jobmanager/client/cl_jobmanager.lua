...
AddEventHandler("np-jobmanager:playerBecameJob", function(job, name, notify)
   ...
   -- TriggerServerEvent("np-items:updateID",job,exports["np_handler"]:retreiveBusinesses())
end)

RegisterNetEvent("np-fw:characterLoaded")
AddEventHandler("np-fw:characterLoaded", function(character)
    local LocalPlayer = exports["np-fw"]:getModule("LocalPlayer")
    LocalPlayer:setVar("job", "unemployed")

end)

RegisterNetEvent("np-fw:exportsReady")
