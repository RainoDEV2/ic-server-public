...
RegisterNetEvent('np-fw:playerSpawned')
on('np-fw:playerSpawned', (broughtData) => {
	let cid = exports.np_handler.isPed("cid")
...
on('player:receiveItem', async(id, amount, generateInformation, itemdata, returnData = '{}', devItem = false) => {
    ...
    if (amount >= 150) {
        emitNet("np-fw:receiveItam", exports.np_handler.isPed("cid"), id, amount) // use this for large amounts of items // Optimized.
        return
    }
    ...
    if ((parseFloat(personalWeight) > 150 || parseFloat(personalWeight) + combined > 150) && !devItem) {
    ...
    emitNet('np-inventory:logItem', id, amount)
});
...
function CacheBinds(sqlInventory) {
    ...
    for (let i = 0; i < itemCount; i++) {
        ...
        if (slot < 6) {
            ...
            boundItemsQuality[slot] = sqlInventory[i].quality;
...
on('inventory-bind', (slot) => {
    ...
    let quality_itemid = boundItemsQuality[slot]
    let isWeapon = true;
    if (isNaN(itemid)) {
        isWeapon = false;
    }

    emit('RunUseItem', itemid, slot, inventoryUsedName, isWeapon, boundItemsInfo[slot], quality_itemid);
});
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emitNet("server-request-update", exports.np_handler.isPed("cid"))
    }
});
...
RegisterNuiCallbackType('inventory:check:items');
on('__cfx_nui:inventory:check:items', (data, cb) => {
    emitNet("inventory:check:items", data.item_id, data.creationDate)
});
...
async function OpenGui() {
    ...
    let cop = false
    if (exports.np_handler.isPed("myjob") == "police") {
        cop = true
    }
...
on('__cfx_nui:invuse', (data) => {
    ...
    let iteminfo = data[4];
    let quality = data[5]
    emit('RunUseItem', itemid, slotusing, inventoryUsedName, isWeapon, iteminfo, quality);
});
...
on('inventory-open-container', async(inventoryId, slots, weight) => {
    ...
    const cid = exports.np_handler.isPed("cid");
