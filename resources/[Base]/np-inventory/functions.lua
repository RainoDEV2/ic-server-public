...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, itemshit, quality)
    if itemid == nil then
        return
    end
    local ItemInfo = GetItemInfo(slot)
    if tonumber(quality) ~= nil then
        if tonumber(quality) < 1 then
            if isWeapon then
                TriggerEvent("DoLongHudText","Weapon is too worn.", 2) 
    ...
    if (itemid == "hunting_bait") then
        TriggerEvent("np-hunting:startHunting")
     end
 
    if (itemid == "hunting_knife") then
        TriggerEvent("np-hunting:startSkinning")
    end
    ...
    if (itemid == "hacklaptop") then
        TriggerEvent('np-paletorob:attempt:hack')
    end

    if (itemid == "electronickit") then
      TriggerEvent("np-paletorob:power:attempt")
    end
    ...
    if (itemid == "taserammo") then
        TriggerEvent("taser:client:refill")
    end

    if (itemid == "paintballs") then
        AddAmmoToPed(PlayerPedId(), 2285322324, 20)
        remove = true
    end
    ...
    if (itemid == "knockoutmeal") then
        local finished = exports["np-taskbar"]:taskBar(7500,"Opening Meal")
        if (finished == 100) then
            remove = true
            TriggerServerEvent("knockoutmeal:give:items")
        end
    end

    if (itemid == "bleedingmeal") then
        local finished = exports["np-taskbar"]:taskBar(7500,"Opening Meal")
        if (finished == 100) then
            TriggerServerEvent("bleedingmeal:give:items1")
            print("done")
            remove = true
        end
    end
    ...
    if (itemid == "ownerreceipt") then
        ...
        if itemInfo ~= nil then
            if itemInfo.information ~= nil then
                ...
                if exports['np-businesses']:NearReceiptShit() then
                local percent = math.ceil(decoded.Price * 0.75)
                decoded.Price = math.ceil(decoded.Price - percent)
                    if myJob == "burger_shot" then
                        TriggerServerEvent('society:update', decoded.Price, "burger_shot", "add")
                        remoteCalls.execute("np-businesses:addcash", percent)
                        remove = true
                    elseif myJob == "best_buds" then
                        TriggerServerEvent('society:update', decoded.Price, "best_buds", "add")
                        remoteCalls.execute("np-businesses:addcash", percent)
                        remove = true
                    elseif myJob == "bean_machine" then
                        TriggerServerEvent('society:update', decoded.Price, "bean_machine", "add")
                        remoteCalls.execute("np-businesses:addcash", percent)
                        remove = true
                    elseif myJob == "vanilla_unicorn" then
                        TriggerServerEvent('society:update', decoded.Price, "vanilla_unicorn", "add")
                        remoteCalls.execute("np-businesses:addcash", percent)
                        remove = true
                    elseif myJob == "bahamas_bar" then
                        TriggerServerEvent('society:update', decoded.Price, "bahamas_bar", "add")
                        remoteCalls.execute("np-businesses:addcash", percent)
                        remove = true
                    else
                    remove = false 
    ...
    if (itemid == "oneshotmeal") then
        local finished = exports["np-taskbar"]:taskBar(7500,"Opening Meal")
        if (finished == 100) then
            TriggerServerEvent("oneshotmeal:give:items")
            remove = true
        end
    end
    ...
    if itemid == "matterspremier" or itemid == "francispremier" then
    ...
    if (itemid == "caged") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["np-taskbar"]:taskBar(4000,"What Could This Be?👅",false,false,playerVeh)
        if (finished == 100) then
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",24,1200)
            TriggerEvent("fx:run", "lsd", 180, nil, (itemid == "badlsdtab" and true or false))
    ...
    if (itemid == "femaleseed") then
       TriggerEvent("np-state:stateSet",4,1600)
    ...
    if (itemid == "joint2") then
        if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
            local finished = exports["np-taskbar"]:taskBar(5000,"Smoking Joint",false,false,playerVeh,false,true)
            local armour =  GetPedArmour(PlayerPedId())
            if (finished == 100) then
                if exports["np-inventory"]:getQuantity("joint2") > 0 then
    ...
    if (itemid == "joint") then
        if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
            ...
            local armour =  GetPedArmour(PlayerPedId())
            if (finished == 100) then
                if exports["np-inventory"]:getQuantity("joint") > 0 then
                    TriggerEvent("client:newStress",false, 1000)
                    TriggerEvent("DoLongHudText", "Stress Relieved")
                    if armour >= 60 then
                        SetPedArmour(PlayerPedId(), 60)
    ...
    if (itemid == "fishingrod") then
        TriggerEvent("np-fish:tryToFish")
    end

    if (itemid == "vodka" or itemid == "beer" or itemid == "whiskey" or itemid == "tequila" or itemid == "redwine" or itemid == "wineglass" or itemid == "rum" or itemid == "tequilashot" or itemid == "whitewine") then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,6000,"Drinking","changethirst",true,itemid,playerVeh)
        TriggerEvent("Evidence:StateSet", 8, 600)
        local alcoholStrength = 0.5
    ...
    if (itemid == "coffee") then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,6000,"Drinking","coffee:drink",true,itemid,playerVeh)
    end

    if (itemid == "bscoffee" or itemid == "frappuccino") then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,6000,"Drinking","coffee:drink2",true,itemid,playerVeh)
    end

    if (itemid == "espresso") then
    ...
    if (itemid == "greencow") then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,6000,"Drinking","food:Condiment",true,itemid,playerVeh)
    end
    ...
    if (itemid == "Gruppe6Card") then
        local police = exports["np_handler"]:isPed("countpolice")
        if police >= 4 then
            ...
            if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) then
                ...
                if aDist < 2.0 then
                    local finished = exports["np-taskbar"]:taskBar(1,"Unlocking Vehicle",false,false,playerVeh)
                    if finished == 100 then
                        remove = true
            ...
            TriggerEvent("DoLongHudText", "Door is to secure to unlock", 2)
        end
    end
    ...
    if (itemid == "armor" or itemid == "pdarmor") then
        ...
        local finished = exports["np-taskbar"]:taskBar(10000,"Putting on Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "fcadrive") then
        TriggerEvent("np-robbery:usb")
    end
    
    if (itemid == "backpack") then
        local finished = exports["np-taskbar"]:taskBar(7500,"Unzipping Backpack")
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "securityblue" or itemid == "securityblack" or itemid == "securitygreen" or itemid == "securitygold" or itemid == "securityred")  then
        ...
        TriggerEvent("np-robbery:UseFleecaCardA")
    end
    ...
    if (itemid == "ciggy") then
        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
            ...
            local finished = exports["np-taskbar"]:taskBar(15000,"Smoking Ciggy",false,false,playerVeh,false,true)
            if (finished == 100) then
                if exports["np-inventory"]:getQuantity("ciggy") > 0 then
                    TriggerEvent("client:newStress", false, 300, false)
                    remove = true
                end
            end
        end
    ...
    if (itemid == "aspirin")  then
        TriggerEvent("animation:PlayAnimation","pill")
        TriggerEvent("client:newStress",false,700)
        remove = true
    end    
    ...
    if (itemid == "oxygentank") then
        local finished = exports["np-taskbar"]:taskBar(60000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then        
    ...
    if (itemid == "cola" or itemid == "sprunk" or itemid == "water") then
    ...
    if (itemid == "jailfood" or itemid == "bleederburger" or itemid == "heartstopper" or itemid == "torpedo" or itemid == "meatfree" or itemid == "moneyshot" or itemid == "fries" or itemid == "ramen" or itemid == "muffin" or itemid == "maccheese" or itemid == "chips") then
    ...
    if (itemid == "watch") then
        TriggerEvent("np-inventory:watch:status")
    end
...
function AttachPropAndPlayAnimation(dictionary,animation,typeAnim,timer,message,func,remove,itemid,vehicle)
    ...
    elseif itemid == "water" or itemid == "cola" or itemid == "sprunk" or itemid == "vodka" or itemid == "whiskey" or itemid == "beer" or itemid == "coffee" or itemid == "softdrink" or itemid == "espresso" then
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["np-taskbarskill"]:taskBar(2200,4)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["np-taskbarskill"]:taskBar(math.random(5000,10000),math.random(10,15))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            ...
            local finished = exports["np-taskbarskill"]:taskBar(1500,math.random(5,10))
            if finished ~= 100 then
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["np-taskbarskill"]:taskBar(timer,math.random(4,7))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
    ...
    if(distance ~= -1 and distance < 5) then
        ...
        if ( IsEntityPlayingAnim(GetPlayerPed(t), "dead", "dead_a", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "amb@code_human_cower_stand@male@base", "base", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "amb@code_human_cower@male@base", "base", 3) or  IsEntityPlayingAnim(GetPlayerPed(t), "random@arrests@busted", "idle_a", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "mp_arresting", "idle", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "random@mugging3", "handsup_standing_base", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "missfbi5ig_22", "hands_up_anxious_scientist", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "missfbi5ig_22", "hands_up_loop_scientist", 3) ) then
            if IsPedArmed(PlayerPedId(), 7) then
                ...
                local finished = exports["np-taskbar"]:taskBar(10000,"Robbing Player")
                if (finished == 100) then
                    t, distance, closestPed = GetClosestPlayer()
                    if(distance ~= -1 and distance < 5) and( IsEntityPlayingAnim(GetPlayerPed(t), "dead", "dead_a", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "amb@code_human_cower_stand@male@base", "base", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "amb@code_human_cower@male@base", "base", 3) or  IsEntityPlayingAnim(GetPlayerPed(t), "random@arrests@busted", "idle_a", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "mp_arresting", "idle", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "random@mugging3", "handsup_standing_base", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "missfbi5ig_22", "hands_up_anxious_scientist", 3) or IsEntityPlayingAnim(GetPlayerPed(t), "missfbi5ig_22", "hands_up_loop_scientist", 3) ) then
...
RegisterCommand('+useKeyFob', function()
    ...
    if hasEnoughOfItem("casinofob",1,false) then
...
Citizen.CreateThread(function()
    exports["np-keymapping"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
end)