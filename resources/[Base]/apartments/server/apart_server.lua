    ...
    if user:getBalance() >= upfront then
        ...
        exports.ghmattimysql:execute('SELECT `housename` FROM __housedata WHERE `housename`= ?', {housename}, function(data)
            if data[1].cid == nil then
                ...
                exports["np-banking"]:UpdateSociety(upfront, "realtor", "add")
            else
...
AddEventHandler('house:givekey', function(house_id,house_model,house_name,target)
    local src = source
    local user = exports["np-fw"]:getModule("Player"):GetUser(tonumber(target))
    ...
    if IsRealtor(src) == "realtor" then
        exports.ghmattimysql:execute('SELECT * FROM __housedata WHERE house_id = ? AND house_model = ?', {hid, model}, function(result)
            if result[1] ~= nil then
                ...
                LogInfo = GetPlayerName(src) .. " Deleted a house | House_id: " .. hid .. " | Deleter CID: " ..cid
                exports['np-fw']:DiscordLog("https://discord.com/api/webhooks/888639372927336520/23dmqat9sEHCtoHCKBqvAI5eabS4LffBhON7-M63vaANQjVui5S7L6q6Ol3vTAiH6Q6i", src, "House Deletion", "", LogInfo)
