...
AddEventHandler('housing:evictHouseAttempt', function()
	if modifying then
		TriggerServerEvent("house:evictHouse",modifyingTable["house_id"],modifyingTable["house_model"],exports["np_handler"]:isPed("cid"))
	else
	...
	while true do
		...
		if backdooroutside["x"] ~= 0.0 and not insidehouse then
			...
			if #(vector3(backdooroutside["x"],backdooroutside["y"],backdooroutside["z"]-0.3) - plyCoords) < 1.5 then
				DrawText3Ds(backdooroutside["x"],backdooroutside["y"],backdooroutside["z"] , '~p~E~s~ to enter house.')
		...
		if backdoorinside["x"] ~= 0.0 and insidehouse then
			...
			if #(vector3(backdoorinside["x"],backdoorinside["y"],backdoorinside["z"]-0.3) - plyCoords) < 1.5 then
				DrawText3Ds( backdoorinside["x"],backdoorinside["y"],backdoorinside["z"] , '~p~E~s~ to leave house.')
				if IsControlJustReleased(2, 38) then
					insidehouse = false
		...
		if clothing["x"] ~= 0.0 and insidehouse then
			if #(vector3(clothing["x"],clothing["y"],clothing["z"]-0.3) - plyCoords) < 1.5 then
				DrawText3Ds( clothing["x"],clothing["y"],clothing["z"] , '[~p~'..Controlkey["housingSecondary"][2]..'~s~] - change characters | /outfits.' )
		...
		if storage["x"] ~= 0.0 and insidehouse then
			if #(vector3(storage["x"],storage["y"],storage["z"]-0.3) - plyCoords) < 1.5 then
				DrawText3Ds( storage["x"],storage["y"],storage["z"] , 'Press ~p~E~s~ to open your stash.')
				if IsControlJustReleased(2, 38) then
...
AddEventHandler('housing:enter', function()
	...
	if modifying then
		TriggerServerEvent("house:enterhouse",exports['np_handler']:isPed('cid'), house_id,house_model,true)
		housenameraid = house_name
	else
		TriggerServerEvent("house:enterhouse",exports['np_handler']:isPed('cid'), house_id,house_model,false)
		housenameraid = "none"
	end
...
robberycoordsMansions = {
	...
	[265] =  { ['x'] = -930.32,['y'] = 19.47,['z'] = 48.52,['h'] = 35.60, ['info'] = ' Rockford Hils' ,['office'] = 2 },
