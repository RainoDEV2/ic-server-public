...
AddEventHandler('drugs:corner', function()
    ...
    if (MyStreetName == "Forum Dr" or MyStreetName == "Brouge Ave" or MyStreetName == "Grove St") and exports["np-inventory"]:hasEnoughOfItem("weedq",1,false) and dst < 500.0 then
...
function SellDrugs(NPC,saleprice, amount)
	...
	local meth = exports["np-inventory"]:hasEnoughOfItem("1gmeth",amount,false)
	if meth and sellingmeth then
		TriggerEvent("inventory:removeItem", "1gmeth", amount)
	end

	local methbad = exports["np-inventory"]:hasEnoughOfItem("methbag",amount,false)
	...
	if sellingcrack or sellingcocaine or sellingweed or sellingmeth or sellingmethbad then
		...
		if math.random(24) < 5 then
			TriggerEvent("np-dispatch:drugsales")
		end
...
RegisterNetEvent("registerenter")
AddEventHandler("registerenter", function()
	InTargettedArea = true
end)


Citizen.CreateThread(function()
	exports['qtarget']:AddBoxZone("smallshit",vector3(1509.13, 3573.66, 38.74), 0.8, 1, {
		name="smallshit",
		heading=350,
		minZ=35.54,
		maxZ=39.54
	...
	exports['qtarget']:AddBoxZone("bigshit",	vector3(1795.01, 4602.61, 37.68), 1.5, 0.5, {
		name="bigshit",
		heading=275,
		minZ=35.08,
		maxZ=39.08
	...
	exports['qtarget']:AddBoxZone("hoyabig",	vector3(-2176.28, 4295.2, 49.05), 1.4, 1, {
		name="hoyabig",
		heading=330,
		minZ=46.65,
		maxZ=50.65
	...
	exports['qtarget']:AddBoxZone("hoyasmall",vector3(-31.04, 1955.23, 190.19), 0.7, 0.5, {
		name="small meth",
		heading=280,
		minZ=186.99,
		maxZ=190.99
...
AddEventHandler('break:smallcoke', function()
	if exports['np-inventory']:hasEnoughOfItem('coke50g', 1) then
		if exports['np-inventory']:hasEnoughOfItem('drugbaggy', 20) then
			amountofcrack = math.random(3, 7)
			exports["np-taskbar"]:taskBar(15000,"Dividing")
...
RegisterNetEvent('prison:task1')
AddEventHandler('prison:task1', function()
	if weld1 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
		FreezeEntityPosition(PlayerPedId(),true)
		local finished = exports['np-taskbar']:taskBar(20000, 'Fixing Electrical Box')
		if finished == 100 then
			weld1 = true
			FreezeEntityPosition(PlayerPedId(),false)
			TriggerServerEvent("jail:cuttime")
			TriggerEvent('DoLongHudText', 'Good Work Shitbag, here is some time off your senteance and some loose change, now get back to work! ')
			ExecuteCommand('e c')
			Citizen.Wait(5000)
			Citizen.Wait(300000)
			weld1 = false
...
RegisterNetEvent('prison:task2')
AddEventHandler('prison:task2', function()
	if weld2 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task3')
AddEventHandler('prison:task3', function()
	if weld3 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task4')
AddEventHandler('prison:task4', function()
	if weld4 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task5')
AddEventHandler('prison:task5', function()
	if weld5 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task6')
AddEventHandler('prison:task6', function()
	if weld6 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task7')
AddEventHandler('prison:task7', function()
	if weld7 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task8')
AddEventHandler('prison:task8', function()
	if weld8 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task9')
AddEventHandler('prison:task9', function()
	if weld9 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task10')
AddEventHandler('prison:task10', function()
	if weld10 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task11')
AddEventHandler('prison:task11', function()
	if weld11 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task12')
AddEventHandler('prison:task12', function()
	if weld12 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task13')
AddEventHandler('prison:task13', function()
	if weld13 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
...
RegisterNetEvent('prison:task14')
AddEventHandler('prison:task14', function()
	if weld14 == true then
		TriggerEvent('DoLongHudText', 'This one isnt faulty yet, stop trying to waste time. Go do some laps and come back!')
	else
		ExecuteCommand('e welding')
