...
RegisterNetEvent("crackstart:menu")
AddEventHandler("crackstart:menu", function()
	TriggerEvent('np-context:sendMenu', {
		{
			id = "1",
			header = "Coke Lab",
			txt = "Bakingsoda | Chloroform | Secret Item",
			params = {
				event = "crack:start",
			}
		},
...
AddEventHandler("crack:start", function()
	...
	if exports["np-inventory"]:hasEnoughOfItem("bakingsoda",3,false) and exports["np-inventory"]:hasEnoughOfItem("chloroform",2,false) and exports["np-inventory"]:hasEnoughOfItem("dye",2,false) and ongoing == false then
		...
		if (finished == 100) then
			...
			if math.random(1,30) == 18  then
				TriggerEvent("inventory:removeItem","bakingsoda", 3)
				TriggerEvent("inventory:removeItem","chloroform", 2)
				TriggerEvent("inventory:removeItem","dye", 2)
...
AddEventHandler("crack:sell", function()
	if exports["np-inventory"]:hasEnoughOfItem("1gcrack",1,false) then
		...
		if (finished == 100) then
			remoteCalls.execute("crack:givemoney")
