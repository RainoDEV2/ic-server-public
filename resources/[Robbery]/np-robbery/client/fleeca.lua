    ...
    if closestBank ~= nil then
        ...
        if not isBusy then
            if closestBank ~= nil then
                ...
                if dist < 1.5 then				
                    if not Config.SmallBanks[closestBank]["isOpened"] then 
                        if exports['np-inventory']:hasEnoughOfItem('hacklaptop', 1) and exports['np-inventory']:hasEnoughOfItem('fcadrive', 1) then
                            if police >= 4 then
                                exports['np-dispatch']:SendAlert("AlertFleecaRobbery")
                                StartHeistFleecaPanel()
                                local card = exports["np-taskbar"]:taskBar(9000,"Hooking up equipment")
                                if card == 100 then
                                    ...
                                    TriggerEvent('open:minigame', 20, 4, function(Success)
                                        ...
                                        if minigameResult then
                                            ...
                                            TriggerEvent('DoShortHudText', 'Congrats! You passed the captcha. Give me a few moments to open the door.')
...
function OpenLocker(bankId, lockerId)
    ...
    TriggerServerEvent('np-robbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "pacific" then
        -- print(exports['np-inventory']:getQuality('hacklaptop'))
        -- if exports['np-inventory']:hasEnoughOfItem('hacklaptop', 1) then
		...
		-- 	TriggerServerEvent('np-robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
		-- end
    else
        ...
        if exports['np-inventory']:hasEnoughOfItem('lockpick', 1) then
            ...
            if finished == 100 then 
                ...
                local finished = exports["np-taskbar"]:taskBar(15000,"Opening")
                if finished == 100 then
                    lockpicking = false
                    TriggerEvent("fleeca:success")
                    FreezeEntityPosition(PlayerPedId(), false)
                    TriggerServerEvent('np-robbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('np-robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent("Fleeca:success:lockpick")
                else
                    lockpicking = false
                    FreezeEntityPosition(PlayerPedId(), false)
                    TriggerServerEvent('np-robbery:server:setLockerState', bankId, lockerId, 'isOpened', false)
                    TriggerServerEvent('np-robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                end
            else
...
RegisterNetEvent("fleeca:success:panel", function()
    ...
    TriggerEvent('np-dispatch:bankrobbery', -1)
    TriggerServerEvent('np-robbery:server:setBankState', closestBank, true)
    TriggerServerEvent('np-robbery:server:setBankStatelockers', closestBank)
    DeleteObject(laptop)
end)
...
function UseBankPanel(panelCoords, panelHeading, location)
    ...
    --     gameDuration = 10000
    -- end
    -- exports["np-interaction"]:openApplication("minigame-captcha", {
