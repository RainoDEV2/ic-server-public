...
RegisterNetEvent('np-robbery:UseBankThermiteOne')
AddEventHandler('np-robbery:UseBankThermiteOne', function(bankId, lockerId)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
	local dist = GetDistanceBetweenCoords(pos, Config.BigBanks["pacific"]["thermite"][1]["x"], Config.BigBanks["pacific"]["thermite"][1]["y"], Config.BigBanks["pacific"]["thermite"][1]["z"])
    ...
    if dist < 1.5 then
		if CheckPolice() >= Config.MinimumPacificPolice then
			if exports['np-inventory']:hasEnoughOfItem('thermite', 1) then
                    if exports["np-thermite"]:startGame(15,2,10,500) then
					...
					TriggerServerEvent("np-doors:alterlockstate",79)
                    else
...
RegisterNetEvent('np-robbery:UseBankThermiteTwo')
AddEventHandler('np-robbery:UseBankThermiteTwo', function(bankId, lockerId)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
		...
		if CheckPolice() >= Config.MinimumPacificPolice then
			if exports['np-inventory']:hasEnoughOfItem('thermite', 1) then
                if exports["np-thermite"]:startGame(15,2,10,500) then
					...
					TriggerServerEvent("np-doors:alterlockstate",80)
                    else
                        local plyCoords = GetEntityCoords(PlayerPedId())
                        TriggerEvent('DoLongHudText', "You notice fire, RUN!", 2)
                        Wait(2000)
				    ...
				    TriggerEvent('inventory:removeItem',"thermite", 1)
					TriggerEvent("np-state:stateSet",16,2200)
            else
                TriggerEvent('DoLongHudText', "You dont have any thermite!", 2)
			end
		else
...
RegisterNetEvent('np-robbery:UseBankLockPick')
AddEventHandler('np-robbery:UseBankLockPick', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
                ...
                if exports['np-inventory']:hasEnoughOfItem('thermite', 1) then
                    ...
                    ClearPedTasks(PlayerPedId())
                      if finished == 100 then
                          ...
                          TriggerServerEvent("np-doors:alterlockstate",76)
                         
                          TriggerEvent('inventory:removeItem', 'lockpick', 1)
                          TriggerEvent("np-state:stateSet",26,1200)
...
RegisterNetEvent('np-robbery:UseBankCardA')
AddEventHandler('np-robbery:UseBankCardA', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if dist < 1.5 then
        ...
        if not isBusy then
            ...
            if dist < 1.5 then
                if CheckPolice() >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then
                        if exports['np-inventory']:hasEnoughOfItem('electronickit', 1) then
                            FreezeEntityPosition(ped, true)
                            ...
                            if card == 100 then
                                FreezeEntityPosition(ped, true)
                                Wait(400)
                                local hack = exports["np-taskbar"]:taskBar(6000,"Hacking..")
                                FreezeEntityPosition(ped, false)
                                Wait(300)
...
function OnHackDoorDone(success, timeremaining)
    if success then
		...
		TriggerEvent('np-dispatch:pacificrobbery', -1)
		TriggerEvent('DoLongHudText', "Success!", 1)
		TriggerServerEvent("np-doors:alterlockstate",78)
    else
		TriggerEvent('mhacking:hide')
		TriggerEvent("animation:cancel")
...
RegisterNetEvent('np-robbery:UseBankCardB')
AddEventHandler('np-robbery:UseBankCardB', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
					    ...
					    local card = exports["np-taskbar"]:taskBar(9000,"Inserting Card")
						    ...
						    if card == 100 then
                                ...
                                if Config.BigBanks["pacific"]["alarm"] then
                                    TriggerEvent('np-dispatch:pacificrobbery')
                                copsCalled = true
                            end
                        end
...
RegisterNetEvent('np-robbery:UseBankCardC')
AddEventHandler('np-robbery:UseBankCardC', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
function OnHackPacificDone(success, timeremaining)
    if success then
		...
		TriggerEvent("animation:cancel")
		TriggerEvent('DoLongHudText', "Success!", 1)
        TriggerServerEvent('np-robbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
		TriggerEvent("animation:cancel")
		TriggerEvent('DoLongHudText', "Failed!", 1)
