...
AddEventHandler("robbery:register", function()
	...
	TriggerEvent("player:receiveItem","rollcash",math.random(20,40))
end)
...
AddEventHandler("robbery:safe", function()
	...
	if tonumber(storeid) == tonumber(CorrectStore) then
		...
		if math.random(20) > 5 then
		    if math.random(100) > 70 then
		      ...
		      TriggerEvent("player:receiveItem","secretnote",1)
		    end
		end

		TriggerEvent("player:receiveItem","band",math.random(20, 35))
	...
	TriggerEvent("player:receiveItem","rollcash",math.random(15, 35))
...
function SpawnPed(i)
    ...
    local IsPedNearCoords = exports["np_handler"]:IsPedNearCoords(x,y,z)
...
Citizen.CreateThread(function()
	exports["np-polyzone"]:AddBoxZone("store_robbery_lucky", vector3(1002.45, -1527.73, 30.84), 2, 2, {
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "store_robbery_lucky" then
        LuckySpot = true     
        LuckyDrawRobberyPlace()
		TriggerEvent('np-textui:ShowUI', 'show', ("[E] %s"):format("Talk"))
    end
end)
...
function LuckyDrawRobberyPlace()
	Citizen.CreateThread(function()
        while LuckySpot do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
				local police = exports["np_handler"]:isPed("countpolice")
				if police >= 3 then
				if exports["np-inventory"]:hasEnoughOfItem("iciest coin",1,false) then
					TriggerEvent("inventory:removeItem", "iciest coin", 1)
					FreezeEntityPosition(PlayerPedId(),true)
					TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_HANG_OUT_STREET", 0, true)
					local finished = exports["np-taskbar"]:taskBar(7500,"Gathering Information")
					...
					exports['np-memory']:StartMinigame({
						...
						FreezeEntityPosition(PlayerPedId(),false)	
						ClearPedTasksImmediately(PlayerPedId())
						TriggerEvent("player:receiveItem", "iciest coin", 1)
					end
				else
