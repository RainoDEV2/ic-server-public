...
AddEventHandler('rich:TakePlayers', function(count)
    ...
    if SetDiscordRichPresenceAction then
        SetDiscordRichPresenceAction(0, 'Discord', 'https://discord.gg/vpRTuKcAU7')
    end
end)

Citizen.CreateThread(function()
    TriggerServerEvent('rich:GetPlayers')
    SetDiscordAppId(827602328915410965)
 	SetDiscordRichPresenceAsset('1000x1000')
    while true do
        Citizen.Wait(2000)
        SetDiscordRichPresenceAssetText("https://discord.gg/vpRTuKcAU7")
        SetRichPresence(onlinePlayers.."/109 | Eating Chicken")
...
-- Afk Kicker
 ...
 RegisterNetEvent("iciest :afk:update")
 AddEventHandler("iciest :afk:update", function(status)
     disabled = status
 end)
    ...
    while true do
        playerPed = PlayerPedId() 
            if IsPedDoingDriveby(playerPed) then
                if GetFollowPedCamViewMode() == 0 or GetFollowVehicleCamViewMode() == 0 or GetFollowVehicleCamViewMode() == 1 or GetFollowVehicleCamViewMode() == 2  then
