...
RegisterServerEvent('np-adminmenu:Disconnect')
AddEventHandler('np-adminmenu:Disconnect', function(reason)
    DropPlayer(source, reason)
end)

RegisterServerEvent('admin:isFlying')
AddEventHandler('admin:isFlying', function(data)
    TriggerEvent('np-adminmenu:NoclipState', data)
end)
...
AddEventHandler('admin:bringPlayer', function(pSrc, target)
    ...
    if pAllowed(pSrc, "Bring") ~= "user" then
        TriggerClientEvent('np-adminmenu:bringPlayer', pTarget, coords)
...
RegisterServerEvent("admin:attempt:spawn:item", function(pSrc, ItemName, ItemAmount)
    ...
    exports["np-fw"]:k_log(pSrc, "Spawned:items", info)
end)

RegisterServerEvent("admin:attempt:spawn:car", function(pSrc, Model, livery)
    local info =  "Vehicle Spawned: " ..string.upper(Model)
    TriggerClientEvent('np-adminmenu:runSpawnCommand', pSrc, Model, livery)
    exports["np-fw"]:k_log(pSrc, "Spawned:car", info)
end)
...
AddEventHandler("admin:getCoords", function(Lsrc, target, toggle)
    ...
    exports["np-fw"]:k_log(pSrc, "Spectating", info)
...
AddEventHandler("admin:search", function(pSrc, tSrc)
    local user = exports["np-fw"]:getModule("Player"):GetUser(tSrc)
    ...
    if pAllowed(pSrc, "Search") ~= "user" then
        ...
        exports["np-fw"]:k_log(pSrc, "Searching", info)
    end
end)


RegisterServerEvent("np-adminmenu:delete:test")
AddEventHandler("np-adminmenu:delete:test", function(pSrc, pCid, pModel)
...
RegisterServerEvent('np-adminmenu:setcloak')
AddEventHandler('np-adminmenu:setcloak', function(args)
    TriggerClientEvent('cloak', source, args)
end)

RegisterServerEvent('np-adminmenu:kick')
AddEventHandler('np-adminmenu:kick', function(kickid, reason)
    DropPlayer(kickid, reason)
end)


RegisterServerEvent('np-adminmenu:AddPlayer')
AddEventHandler("np-adminmenu:AddPlayer", function()
    local licenses
    ...
    TriggerClientEvent("np-adminmenu:AddPlayer", -1, data )
...
function IRP.Admin.AddAllPlayers(self)
    ...
    for _, playerId in ipairs(xPlayers) do
        ...
        TriggerClientEvent("np-adminmenu:AddAllPlayers", source, data)
...
AddEventHandler("playerDropped", function()
    ...
    TriggerClientEvent("np-adminmenu:RemovePlayer", -1, data )
    Wait(600000)
    TriggerClientEvent("np-adminmenu:RemoveRecent", -1, data)
end)
...
RegisterServerEvent('np-adminmenu:runCommand')
AddEventHandler('np-adminmenu:runCommand', function(data)
    local src = source
    TriggerClientEvent('np-adminmenu:RunClCommand', src, data.command, data)
...
AddEventHandler('admin:teleportToPlayer', function(target)
    ...
    if pAllowed(src, "Teleport") ~= "user" then
        TriggerClientEvent('np-adminmenu:bringPlayer', src, coords)
...
RegisterServerEvent("np-adminmenu:update:vehicle")
AddEventHandler("np-adminmenu:update:vehicle", function(pSrc, vPlate, vGarage)
...
function pAllowed(pSrc, pTrigger)
    ...
    exports.ghmattimysql:execute("SELECT `rank` FROM users WHERE hex_id = ?", {hexId}, function(data)
        ...
        if pRank == "user" then
            ...
            exports['np-fw']:DiscordLog("https://discord.com/api/webhooks/888637988911861760/hBwvp2tKY0AMgXrZvU4dqQz3oP1szvKmI1D24sm4bidHm8JueiBmgMvgKDV8NE19w0p9", pSrc, "Trigger-Event-Admin", "Triggering Events", LogInfo)
