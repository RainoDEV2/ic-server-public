...
function IRP.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return exports["np-fw"]:getModule("LocalPlayer"):getVar("rank")
    else
...
function IRP.Admin.IsValidUser(self, user)
    ...
    local util = exports["np-fw"]:getModule("Util")
...
function IRP.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        for k,v in pairs(IRP._Admin.CurAdmins) do
            TriggerClientEvent("np-adminmenu:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("np-adminmenu:setStatus", status)
end
...
AddEventHandler("np-fw:exportsReady", function()
    exports["np-fw"]:addModule("Admin", IRP.Admin)
    exports["np-fw"]:addModule("_Admin", IRP._Admin)
end)