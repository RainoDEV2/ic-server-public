...
function cmd.RunClCommand(args)
    ...
    TriggerEvent("np-hud:godCheck",args.toggle)
end
...
function cmd.Init()
    ...
    RegisterNetEvent("np-adminmenu:NoclipState")
    AddEventHandler("np-adminmenu:NoclipState", function(toggle)
...
function cmd.RunCommand(caller, args)
    ...
    TriggerEvent("np-adminmenu:update:vehicle", src, args.message, args.garage)
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerClientEvent("np-adminmenu:Cloak", -1, src, args.toggle)
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then 
        AddEventHandler("np-fw:characterLoaded", function(user, char)
            ...
            TriggerClientEvent("np-adminmenu:Cloak", src, cmd.vars.cloaked)
        end)

        AddEventHandler("playerDropped", function()
            local src = source
            if cmd.vars.cloaked[src] then
                TriggerClientEvent("np-adminmenu:Cloak", -1, src, false)
    ...
    -- RegisterNetEvent("np-adminmenu:CloakRemote")
    -- AddEventHandler("np-adminmenu:CloakRemote", function()
    ...
    -- RegisterNetEvent("np-adminmenu:Cloak")
    -- AddEventHandler("np-adminmenu:Cloak", function(player, toggle)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Seat into Vehicle") then TriggerEvent("np-adminmenu:SeatIntoLast") end
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerEvent("np-adminmenu:delete:test", src, args.reciver, args.message)
...
function cmd.RunCommand(caller, args)
    ...
    if args.time ~= nil and args.time ~= 0 then
        TriggerEvent("np-weathersync:server:setTime",args.time)
        -- TriggerEvent("weather:time",src,args.time)
    end

    if args.weather ~= nil and args.weather ~= "" then
        TriggerEvent("np-weathersync:server:setWeather", args.weather)
    end
end
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerClientEvent('np-clothingmenu:AdminSetModel', src, args.message)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerEvent("np-adminmenu:ReviveInDistance")
    end
...
--[[
    function cmd.ChatCommand(args)
        ...
        local args = {
            target = exports["np-fw"]:getModule("Player"):getUser(target),
            rank = rank
        }
