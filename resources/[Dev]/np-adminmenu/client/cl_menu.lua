...
function IRP.Admin.Menu.DrawJobs(self, cmd, cb)
    ...
    local jobs = exports["np-fw"]:getModule("JobManager").ValidJobs;
    Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    ...
    local function DrawDev()
        ...
        if WarMenu.Button("Dev mode: " .. (IRP._Admin.Menu.DevMode and "Disable" or "Enable")) and userRank == "dev" then 
            ...
            TriggerEvent("np-adminmenu:currentDevmode",IRP._Admin.Menu.DevMode)
        end
        if WarMenu.Button("Dev debug: " .. (IRP._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "dev" then 
            ...
            TriggerEvent("np-adminmenu:currentDebug",IRP._Admin.Menu.DevDebug)
    ...
    --             TriggerServerEvent("np-adminmenu:searchRequest", IRP._Admin.SearchOptions.type, result)
    --         end)
    --     end
...
RegisterNetEvent("np-adminmenu:openMenu")
AddEventHandler("np-adminmenu:openMenu", function()
    WarMenu.OpenMenu("amenu")
end)

RegisterNetEvent("np-adminmenu:drawLastCat")
AddEventHandler("np-adminmenu:drawLastCat", function(sentCat)
    cat = sentCat
    WarMenu.OpenMenu("acommands")
end)
...
RegisterCommand('menu', function()
    if IRP.Admin:GetPlayerRank() == "dev" or "moderator" then
        print(IRP.Admin:GetPlayerRank())
        TriggerEvent('np-adminmenu:openMenu')
    else 
        TriggerEvent("DoLongHudText","You are not a admin!",2)
    end 
end)
