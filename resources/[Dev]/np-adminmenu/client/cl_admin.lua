...
RegisterNetEvent("np-adminmenu:setStatus")
AddEventHandler("np-adminmenu:setStatus", function(src, status)
    local player = IRP._Admin.Players[src]
    if not player then return else IRP._Admin.Players[src].status = status end
end)

RegisterNetEvent("np-adminmenu:sendPlayerInfo")
AddEventHandler("np-adminmenu:sendPlayerInfo", function(data, discData)
    IRP._Admin.Players = data
    IRP._Admin.DiscPlayers = discData
end)

RegisterNetEvent("np-adminmenu:RemovePlayer")
AddEventHandler("np-adminmenu:RemovePlayer", function(data)
    IRP._Admin.DiscPlayers[data.source] = data
    IRP._Admin.Players[data.source] = nil
end)

RegisterNetEvent("np-adminmenu:AddPlayer")
AddEventHandler("np-adminmenu:AddPlayer", function(player)
    IRP._Admin.Players[player.source] = player
end)

RegisterNetEvent("np-adminmenu:AddAllPlayers")
AddEventHandler("np-adminmenu:AddAllPlayers", function(data)
...
AddEventHandler('event:control:adminDev', function(useID)
    ...
    if IRP.Admin:GetPlayerRank() == "dev" then
        if useID == 1 then
            TriggerEvent("np-adminmenu:openMenu")
        elseif useID == 2 then
            local bool = not isInNoclip
            IRP.Admin.RunNclp(nil,bool)
            TriggerEvent("np-adminmenu:noClipToggle",bool)
            TriggerServerEvent("admin:noclipFromClient",bool)
        elseif useID == 3 then
            TriggerEvent("np-adminmenu:CloakRemote")
        elseif useID == 4 then
...
function IRP.Admin.RunCommand(self, args)
    ...
    TriggerServerEvent("np-adminmenu:runCommand", args)
end
...
function IRP.Admin.RunNclp(self,bool)
    ...
    TriggerEvent("np-adminmenu:noClipToggle", isInNoclip)
end
...
RegisterNetEvent("np-adminmenu:RunClCommand")
AddEventHandler("np-adminmenu:RunClCommand", function(cmd, args)
    IRP.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("np-adminmenu:updateData")
AddEventHandler("np-adminmenu:updateData", function(src, type, data)
...
RegisterNetEvent("np-adminmenu:noLongerAdmin")
AddEventHandler("np-adminmenu:noLongerAdmin", function()
    IRP._Admin.Players = {}
...
RegisterNetEvent("np-adminmenu:bringPlayer")
AddEventHandler("np-adminmenu:bringPlayer", function(targPos)
...
RegisterNetEvent("np-adminmenu:SeatIntoLast")
AddEventHandler("np-adminmenu:SeatIntoLast", function()
    local rank = IRP.Admin:GetPlayerRank()
    local rankData = IRP.Admin:GetRankData(rank)
...
RegisterNetEvent("np-adminmenu:ReviveInDistance")
AddEventHandler("np-adminmenu:ReviveInDistance", function()
    local rank = IRP.Admin:GetPlayerRank()
    local rankData = IRP.Admin:GetRankData(rank)
    ...
    Citizen.CreateThread(function()
        if toggle == true then 
            ...
            if TargetC ~= nil then
                ...
                print("[np-adminmenu]: No Target Coords")
            end
        else
            TargetC = nil
            called = false
