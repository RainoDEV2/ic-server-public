...
function GetVehicleDescription(vehicle)
    local currentVehicle = GetVehiclePedIsIn(PlayerPedId(), false) or vehicle
...
function GetVehicleDescription2(vehicle)
  local currentVehicle = NetworkGetEntityFromNetworkId(vehicle)
  if not DoesEntityExist(currentVehicle) then
    return
  end
...
RegisterNetEvent('boostingAlert')
AddEventHandler('boostingAlert', function(coords, id, plate)
  AlertBoosting(coords, id, plate)
end)
...
function AlertBoosting(coords, netid, plate)
  local vehicleData = GetVehicleDescription2(netid) or {}
  local plyPos = GetEntityCoords(PlayerPedId())
  TriggerServerEvent('dispatch:svNotify', {
    dispatchCode = 'BaitCar',
    relatedCode = initialTenCode,
    firstStreet = street1,
    gender = gender,
    model = vehicleData.model,
    ...
    heading = vehicleData.heading,
    eventId = eventId,
    dispatchMessage = "Bait Car Trigged | Location Update!",
    recipientList = {
      police = "police"
    },
    origin = {
  ...
  TriggerEvent('np-alerts:boost', coords, plate)
end
...
RegisterNetEvent("np-dispatch:alert", function(pType)
  if pType == "AlertFleecaRobbery" then
    AlertFleecaRobbery()
  elseif pType == "AlertDigitalRobbery" then
    AlertDigitalRobbery()
  end
end)
...
function AlertDigitalRobbery()
  ...
  TriggerServerEvent('dispatch:svNotify', {
    ...
    isImportant = true,
    priority = 1,
    dispatchMessage = "Digital Den Robbery",
    recipientList = {
      police = "police"
    },
    origin = {
  ...
  TriggerEvent("np-dispatch:digital:den", plyPos)
end
