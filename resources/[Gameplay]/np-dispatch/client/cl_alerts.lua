RegisterNetEvent('np-outlawalert:gunshotInProgress')
AddEventHandler('np-outlawalert:gunshotInProgress', function(targetCoords)
...
RegisterNetEvent('np-alerts:boosting')
AddEventHandler('np-alerts:boosting', function(targetCoords, plate)
	local job = exports["np_handler"]:isPed("myjob")
	if job == 'police' then
		local alpha = 250
		...
		SetBlipSprite(car, 229)
		SetBlipColour(car, 1)
		SetBlipScale(car, 1.2)
		SetBlipAsShortRange(car,  1)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('Bait Car | Plate: ' .. plate)
...
AddEventHandler('np-alerts:boost', function(coords, plate)
	local pos = GetEntityCoords(PlayerPedId(), true)
	TriggerServerEvent('np-alerts:boosting', {x = coords.x, y = coords.y, z = coords.z}, plate)
end)
...
AddEventHandler('np-dispatch:digital:den', function(pCoords)
	TriggerServerEvent('np-dispatch:fleeca:bank', pCoords)
end)
