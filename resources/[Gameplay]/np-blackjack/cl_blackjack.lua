            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        exports["np-doors"]:showInteraction("Press [E] to play blackjack", 'info')
                    else 
                        exports["np-doors"]:showInteraction("This seat is taken", 'danger')
                        Citizen.Wait(3000)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            exports["np-doors"]:showInteraction("Your hand is["..tostring(currentHand).."], the dealers hand is ["..tostring(dealersHand).."]")
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            TriggerEvent("np-context:closemenu")
        end
    end)
end)
...
AddEventHandler("betmenu", function()
    local tmpInput = exports["np-keyboard"]:KeyboardInput({
        header = "Bet Amount",
        rows = {
            {
                id = 1,
