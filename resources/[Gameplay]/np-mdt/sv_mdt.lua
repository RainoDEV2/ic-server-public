...
RegisterServerEvent("np-mdt:Open")
AddEventHandler("np-mdt:Open", function()
	local usource = source
	local user = exports["np-fw"]:getModule("Player"):GetUser(usource)
    		...
    		exports.ghmattimysql:execute("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    			...
    			exports.ghmattimysql:execute("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
					...
					exports.ghmattimysql:execute('SELECT * FROM characters WHERE id = @id', { ['@id'] = cid }, function(result2)
						...
						TriggerClientEvent('np-mdt:toggleVisibilty', usource, reports, warrants, officer, job)
					end)
    			end)
    		end)
		end
...
RegisterServerEvent("np-mdt:getOffensesAndOfficer")
AddEventHandler("np-mdt:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	local user = exports["np-fw"]:getModule("Player"):GetUser(usource)
	local cid = user:getCurrentCharacter().id
	exports.ghmattimysql:execute('SELECT * FROM fine_types', {
		...
		exports.ghmattimysql:execute('SELECT * FROM characters WHERE id = @id', { ['@id'] = cid }, function(result2)
			local officer = result2[1].first_name ..' '.. result2[1].last_name
			TriggerClientEvent("np-mdt:returnOffensesAndOfficer", usource, charges, officer)
		end)
	end)
end)

RegisterServerEvent("np-mdt:performOffenderSearch")
AddEventHandler("np-mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	exports.ghmattimysql:execute("SELECT * FROM `characters` WHERE LOWER(`first_name`) LIKE @query OR LOWER(`last_name`) LIKE @query OR CONCAT(LOWER(`first_name`), ' ', LOWER(`last_name`)) LIKE @query", {
		...
		TriggerClientEvent("np-mdt:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("np-mdt:getOffenderDetails")
AddEventHandler("np-mdt:getOffenderDetails", function(offender)
	local usource = source
			...
			exports.ghmattimysql:execute('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', { ['@id'] = offender.id }, function(warrants)
				...
				exports.ghmattimysql:execute('SELECT `phone_number` FROM `characters` WHERE `id` = @id', { ['@id'] = offender.id }, function(phone_number)
					...
					exports.ghmattimysql:execute('SELECT * FROM `characters_cars` WHERE `cid` = @cid', { ['@cid'] = offender.id }, function(vehicles)
						...
						TriggerClientEvent("np-mdt:returnOffenderDetails", usource, offender)
					end)
				end)
			end)
		end)
...
RegisterServerEvent("np-mdt:getOffenderDetailsById")
AddEventHandler("np-mdt:getOffenderDetailsById", function(char_id)
	local usource = source
	...
	exports.ghmattimysql:execute('SELECT * FROM `characters` WHERE `id` = @id', { ['@id'] = char_id }, function(result)
		...
		if not offender then
			TriggerClientEvent("np-mdt:closeModal", usource)
			TriggerClientEvent("np-mdt:sendNotification", usource, "This person no longer exists.")
			return
		end
...
RegisterServerEvent("np-mdt:saveOffenderChanges")
AddEventHandler("np-mdt:saveOffenderChanges", function(id, changes, identifier)
	local usource = source
	exports.ghmattimysql:execute('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		TriggerClientEvent("np-mdt:sendNotification", usource, "Offender changes have been saved.")
	end)
end)

RegisterServerEvent("np-mdt:saveReportChanges")
AddEventHandler("np-mdt:saveReportChanges", function(data)
	exports.ghmattimysql:execute('UPDATE `mdt_reports` SET `title` = @title, `incident` = @incident WHERE `id` = @id', {
		['@id'] = data.id,
		['@title'] = data.title,
		['@incident'] = data.incident
	})
	TriggerClientEvent("np-mdt:sendNotification", source, "Report changes have been saved.")
end)

RegisterServerEvent("np-mdt:deleteReport")
AddEventHandler("np-mdt:deleteReport", function(id)
	exports.ghmattimysql:execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
	TriggerClientEvent("np-mdt:sendNotification", source, "Report has been successfully deleted.")
end)

RegisterServerEvent("np-mdt:submitNewReport")
AddEventHandler("np-mdt:submitNewReport", function(data)
	local usource = source
	local user = exports["np-fw"]:getModule("Player"):GetUser(usource)
	local cid = user:getCurrentCharacter().id
	exports.ghmattimysql:execute('SELECT * FROM characters WHERE id = @id', { ['@id'] = cid }, function(result2)
		...
		MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
			...
			TriggerEvent("np-mdt:getReportDetailsById", id, usource)
			TriggerClientEvent("np-mdt:sendNotification", usource, "A new report has been submitted.")
		end)
...
RegisterServerEvent("np-mdt:performReportSearch")
AddEventHandler("np-mdt:performReportSearch", function(query)
	local usource = source
	local matches = {}
	exports.ghmattimysql:execute("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("np-mdt:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("np-mdt:performVehicleSearch")
AddEventHandler("np-mdt:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
...
--- SEARCH MDT PLATE
		...
		TriggerClientEvent("np-mdt:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("np-mdt:performVehicleSearchInFront")
AddEventHandler("np-mdt:performVehicleSearchInFront", function(query)
	local usource = source
	local user = exports["np-fw"]:getModule("Player"):GetUser(usource)
	local cid = user:getCurrentCharacter().id
    			...
    			exports.ghmattimysql:execute("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    				...
    				exports.ghmattimysql:execute("SELECT * FROM `characters_cars` WHERE `license_plate` = @query", {
						...
						exports.ghmattimysql:execute('SELECT * FROM characters WHERE id = @id', { ['@id'] = cid }, function(result2)
							local officer = result2[1].first_name ..' '.. result2[1].last_name
    						TriggerClientEvent('np-mdt:toggleVisibilty', usource, reports, warrants, officer, job)
							TriggerClientEvent("np-mdt:returnVehicleSearchInFront", usource, result, query)
						end)
					end)
    			end)
    		end)
...
RegisterServerEvent("np-mdt:getVehicle")
AddEventHandler("np-mdt:getVehicle", function(vehicle)
	local usource = source
	...
	TriggerClientEvent("np-mdt:returnVehicleDetails", usource, vehicle)
end)

RegisterServerEvent("np-mdt:getWarrants")
AddEventHandler("np-mdt:getWarrants", function()
	local usource = source
	exports.ghmattimysql:execute("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("np-mdt:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("np-mdt:submitNewWarrant")
AddEventHandler("np-mdt:submitNewWarrant", function(data)
	local usource = source
	local user = exports["np-fw"]:getModule("Player"):GetUser(usource)
	local cid = user:getCurrentCharacter().id
	exports.ghmattimysql:execute('SELECT * FROM characters WHERE id = @id', { ['@id'] = cid }, function(result2)
		...
		exports.ghmattimysql:execute('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
			...
			TriggerClientEvent("np-mdt:completedWarrantAction", usource)
			TriggerClientEvent("np-mdt:sendNotification", usource, "A new warrant has been created.")
		end)
	end)
end)

RegisterServerEvent("np-mdt:deleteWarrant")
AddEventHandler("np-mdt:deleteWarrant", function(id)
	local usource = source
	...
	TriggerClientEvent("np-mdt:sendNotification", usource, "Warrant has been successfully deleted.")
end)

RegisterServerEvent("np-mdt:getReportDetailsById")
AddEventHandler("np-mdt:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("np-mdt:returnReportDetails", usource, result[1])
		else
			TriggerClientEvent("np-mdt:closeModal", usource)
			TriggerClientEvent("np-mdt:sendNotification", usource, "This report cannot be found.")
		end
	end)
end)

RegisterServerEvent("np-mdt:saveVehicleChanges")
AddEventHandler("np-mdt:saveVehicleChanges", function(data)
	...
	exports.ghmattimysql:execute('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
		...
		TriggerClientEvent("np-mdt:sendNotification", usource, "Vehicle changes have been saved.")
	end)
end)
