...
RegisterServerEvent('np_luckywheel:getLucky')
AddEventHandler('np_luckywheel:getLucky', function(money)
    local src = source
        ...
        if user ~= nil then
            if user:getCash() >= amount  then
                ...
                SetTimeout(7000, function()
                    ...
                    TriggerClientEvent("np_luckywheel:rollFinished", -1)
                end)
                TriggerClientEvent("np_luckywheel:doRoll", -1, _priceIndex)
            else
                TriggerClientEvent("np_luckywheel:rollFinished", -1)
...
RegisterServerEvent("attempt:spin", function()
    local pSrc = source
    if CasinoSpin then
        TriggerClientEvent("np_luckywheel:spinit", pSrc, true)
    else
