	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
			...
			if IsControlJustReleased(0,38) then
				...
				local finished = exports["np-taskbar"]:taskBar(4000,"Handing Over Package")
...
Citizen.CreateThread(function()
	exports["np-polyzone"]:AddBoxZone("oxy_run_start",vector3(-1328.27, -238.0, 42.69), 1, 1, {
		name="oxy_run_start",
		heading=125,
		--debugPoly=true,
		minZ=41.69,
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "oxy_run_start" then
        ...
        LuckyDrawOxyPlace()
		if not OxyRun then
			TriggerEvent('np-textui:ShowUI', 'show', ("[E] %s"):format("Start Run ($350)"))
		else
...
function DoDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if OxyRun then
				...
				elseif exports["np-inventory"]:hasEnoughOfItem("cashstack",cashstackamount) then
