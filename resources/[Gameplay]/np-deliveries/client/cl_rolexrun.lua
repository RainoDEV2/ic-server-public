...
local stolen8ctchaincount = 5
local stolen10ctchaincount = 5
local stolen2ctchaincount = 5
local goldcoincount = 5
...
local carspawns2 = {
	[1] =  { ['x'] = 414.66665649414,['y'] = -2067.9206542969,['z'] = 20.894361495972,['h'] = 140.40969848633, ['info'] = ' car 1' },
	[2] =  { ['x'] = 412.30355834961,['y'] = -2066.2561035156,['z'] = 20.859544754028,['h'] = 140.51560974121, ['info'] = ' car 2' },
	[3] =  { ['x'] = 410.38494873047,['y'] = -2063.8525390625,['z'] = 20.830171585083,['h'] = 141.20520019531, ['info'] = ' car 3' },
}
...
AddEventHandler("rolexdelivery:startDealing", function()
	local NearNPC = exports["np_handler"]:GetClosestNPC()
...
Citizen.CreateThread(function()
	exports["np-polyzone"]:AddBoxZone("rolex_run_start", vector3(420.08, -2064.09, 22.09), 1.8, 1, {
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "rolex_run_start" then
		...
		if not RolexRun then
			TriggerEvent('np-textui:ShowUI', 'show', ("[E] %s"):format("Start Run ($500s)"))
		else
			TriggerEvent('np-textui:ShowUI', 'show', ("%s"):format("Finish Your Run"))
		end
    end
end)
...
function LuckyDrawRolexPlace()
	Citizen.CreateThread(function()
        while RolexSpot do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
                if exports["np-inventory"]:hasEnoughOfItem("rolexwatch",10) or exports["np-inventory"]:hasEnoughOfItem("goldbar",10) or exports["np-inventory"]:hasEnoughOfItem("bdiamond",1) or exports["np-inventory"]:hasEnoughOfItem("stolen8ctchain",5) or exports["np-inventory"]:hasEnoughOfItem("stolen8ctchain",5) or exports["np-inventory"]:hasEnoughOfItem("stolen10ctchain",5) or exports["np-inventory"]:hasEnoughOfItem("stolen2ctchain",5) or exports["np-inventory"]:hasEnoughOfItem("stolen2ctchain",5) then
                    if not RolexRun then
...
function DoRolexDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if RolexRun then
		        ...
		        if exports["np-inventory"]:hasEnoughOfItem("rolexwatch",rolexcount) then
				...
				elseif exports["np-inventory"]:hasEnoughOfItem("bdiamond",bdiamondcount) then
					...
					TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["np-inventory"]:hasEnoughOfItem("stolen8ctchain",stolen8ctchaincount) then
					...
					TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["np-inventory"]:hasEnoughOfItem("stolen10ctchain",stolen10ctchaincount) then
					...
					TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["np-inventory"]:hasEnoughOfItem("stolen2ctchain",stolen2ctchaincount) then
					...
					TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["np-inventory"]:hasEnoughOfItem("goldcoin",goldcoincount) then
...
function setPed()
	...
	created_ped = CreatePed(0, modelHash , 420.39700317383, -2064.3171386719, 22.142721176147 -1, true)
	FreezeEntityPosition(created_ped, true)
	SetEntityHeading(created_ped, 66.539100646973)
