...
RegisterNetEvent("np-vehicleshop:returnTable")
AddEventHandler("np-vehicleshop:returnTable", function(newTable)
...
RegisterNetEvent("np-vehicleshop:setPlate")
AddEventHandler("np-vehicleshop:setPlate", function(vehicle, plate)
...
AddEventHandler('vehshop:spawnVehicle', function(v)
	...
	if playerPed and playerPed ~= -1 then
		...
		SetEntityInvincible(veh, true)
		TriggerEvent('np-vehicleshop:setPlate', veh, plate)
	end
end)

RegisterNetEvent("np-vehicleshop:update:plate")
AddEventHandler("np-vehicleshop:update:plate", function(plate)
	local veh = GetVehiclePedIsUsing(PlayerPedId())
...
Citizen.CreateThread(function()
    while true do
        ...
        if insideVehShop and isExportReady then
            rank = exports["np_handler"]:GroupRank("car_shop")
            Citizen.Wait(10000)
        end
    end
end)
...
RegisterNetEvent("np-vehicleshop:repo:success")
AddEventHandler("np-vehicleshop:repo:success", function()
...
AddEventHandler("search:list:repo", function()
	...
	if valid then
		TriggerServerEvent("np-vehicleshop:checkrepo", valid[1].input)
	end
end)
