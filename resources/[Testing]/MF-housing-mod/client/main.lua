      ...
      if not self.CurHouse then
        ...
        if pos then
          ...
          if dist and dist < (self.DrawTextDist/2) then
            ESX.Game.Utils.DrawText3D(pos, "Press [~r~E~s~] to access the wardrobe.", 0.7)
        ...
        if tryDoor then
          ...
          if dist and dist < (self.DrawTextDist/2) then
            ESX.Game.Utils.DrawText3D(pos, "Press [~r~E~s~] to leave.", 0.7)
            if dist < self.InteractDist and IsControlJustPressed(0, 38) then
              self:LeaveHouse()
            end
          end
...
function housing:GetActionInfo(closest,closestAct)
  ...
  if closestAct == "Entry" then
    if closest.owner and closest.owner == self.PlayerData.identifier then
      text = "Press [~r~E~s~] to enter your house."
      isOwned = true
    elseif closest.owner and closest.owner ~= self.PlayerData.identifier then
      text = "Press [~r~E~s~] to knock on the door.\nPress [~r~G~s~] to break in."
      isOwned = false
    else
      text = "[$~r~"..closest.Price.."~s~] Press [~r~E~s~] to purchase this house."
      isOwned = false
    end

    if closest.keys then
      text = "Press [~r~E~s~] to enter the house."
      hasKeys = true
    end
  ...
  elseif closestAct == "Garage" then
    if closest.owner and closest.owner == self.PlayerData.identifier then
      if IsPedInAnyVehicle(GetPlayerPed(-1)) then
        ...
        text = "Press [~r~E~s~] to access your garage."
      end
      isOwned = true
    elseif closest.owner and closest.owner ~= self.PlayerData.identifier then
...
function housing:EnterHouse(pos,house,isOwned,firstLogin)
  ...
  if not self.CurHouse.wardrobe and self.PlayerData.identifier == house.owner then
    ESX.ShowNotification("You havn't set a wardrobe position yet!")
    ESX.ShowNotification("Use the /setwardrobe command to set the location.")
  end
...
function housing:InvitePlayer(id)
  if not self.CurHouse or not self.LastPos then ESX.ShowNotification("You aren't inside a house."); return; end
  if not id or tonumber(id[1]) <= 0 then ESX.ShowNotification("You need to enter a target ID."); return; end
  ...
  if dist and dist < 50.0 then
    ...
    ESX.ShowNotification("Player is not near your house.")
  end
end
...
function housing:InviteInside(pos)
  Citizen.CreateThread(function(...)
    ...
    ESX.ShowNotification("Press [~r~H~s~] to accept the invite.")
...
function housing:PlayerKnocked(player)
  if self.CurHouse and self.CurHouse.owner and self.CurHouse.owner == self.PlayerData.identifier then
    ESX.ShowNotification("Somebody just knocked at your door [ ID : "..tostring(player).." ]")
  end
end
