...
function furnicatalogue:PostData(data)
  ...
  if self.ItemLookup[newString] then
    ESX.TriggerServerCallback('furnicatalogue:TryBuy', function(canBuy)    
      if canBuy then
        TriggerEvent('objectspawner:DoPlace',self.ItemLookup[newString])
        self:OpenCatalogue();
      else
        ESX.ShowNotification("You can't afford that.")
      end
...
RegisterCommand('furni', function(...) furnicatalogue:OpenCatalogue(...); end)
