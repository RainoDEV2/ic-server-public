...
AddEventHandler("docks:manage", function(arg,arg2)
  local rank = exports["np_handler"]:GroupRank("dock_worker")
  if rank > 3 then
    if arg == "list" then
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(1)
    if exports["np_handler"]:isPed("myjob") and existingVeh ~= nil then
...
Citizen.CreateThread(function()
  while true do
    Wait(9000)
    if job ~= 0 and exports["np_handler"]:isPed("myjob") then
      currentJobPos = returnJobPos(job)
      checkDestruction()
    end
  end
