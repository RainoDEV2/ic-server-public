		...
		if onJob and onJob == 1 and (isTowVehicle(vehicle) == "towtruck" or isTowVehicle(vehicle) == "towtruck2")  then
			...
			if targetVehicle ~= 0 or not targetVehicle then
				...
				if distToTowYard < 120.0 then
					if pay then 
						TriggerServerEvent("server:givepayJob", "Tow Truck Car Delivery Payment", math.random(20,25)) 
					end
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
        	...
        	if finished == 100 and aDist < 2.0 then
				...
				if CalculateTravelDistanceBetweenPoints( location, -144.85, -1171.88, 23.76 ) < calc then
					calc = CalculateTravelDistanceBetweenPoints( location, -144.85, -1171.88, 23.76 )
				end

				if CalculateTravelDistanceBetweenPoints( location, -192.73, -1175.37, 23.04 ) < calc then
					calc = CalculateTravelDistanceBetweenPoints( location, -192.73, -1175.37, 23.04 )
				end

				if CalculateTravelDistanceBetweenPoints( location, -240.9, -1173.09, 23.04 ) < calc then
					calc = CalculateTravelDistanceBetweenPoints( location, -240.9, -1173.09, 23.04 )
				end
...
AddEventHandler('spawntowtruck', function()
	...
	personalvehicle = CreateVehicle('flatbed',-193.24397277832, -1183.6242675781, 23.044046401978, 270.10357666016, true,false)
	...
	TriggerEvent("DoLongHudText","Use /tow to tow a vehicle", 2)
end)