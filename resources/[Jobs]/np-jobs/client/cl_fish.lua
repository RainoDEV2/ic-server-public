...
AddEventHandler("sellfish", function()
    if exports["np-inventory"]:hasEnoughOfItem("sushiroll",5,false) then
        local finished = exports["np-taskbar"]:taskBar(4000,"Selling Fish")
        if finished == 100 then
            local sushi = math.random(35, 55)
            TriggerEvent("inventory:removeItem", "sushiroll", 5)
            TriggerServerEvent('np-fish:sellSushi', sushi)
        end
    end
end)
...
local blips = {
    ...
    {title="La Spada", colour=8, id=317, scale=0.7, x = -1841.9995117188, y = -1199.3245849609, z = 14.304761886597}
}
...
RegisterNetEvent("np-fish:tryToFish")
AddEventHandler("np-fish:tryToFish", function()
    if poleTimer == 0 then 
        TryToFish()
    end
end)
