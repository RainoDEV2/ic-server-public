...
Citizen.CreateThread(function()
	while true do
		...
		TriggerServerEvent("np-fw:updatecoords",coords.x,coords.y,coords.z)
		Wait(13000)
	end
end)
...
AddEventHandler('police:noLongerCop', function()
	if isInService then
		...
		TriggerEvent("DoLongHudText",'You are already off duty!',1)
	end
end)
...
Citizen.CreateThread(function() 
  ...
  while true do
    ...
    if handCuffedWalking or handCuffed then
		...
		DisableControlAction(1, 37, true) --Disables INPUT_SELECT_WEAPON (tab) Actions
		DisablePlayerFiring(PlayerPedId(), true) -- Disable weapon firing
		local dead = exports["np_handler"]:isPed("dead")
		local intrunk = exports["np_handler"]:isPed("intrunk")
...
RegisterCommand('carry',function()
	TriggerEvent('dragPlayer') 
	TriggerEvent("DoLongHudText","Press E to drop Player!",2)
end)
...
AddEventHandler('impoundVeh', function()
	...
	if job == 'police' or job == 'ems' then
		TriggerEvent('np-context:sendMenu', {
			{
				id = 1,
				header = "Scuff Impound Vehicle",
				txt = "Send Vehicle Back To Garage (T)",
				params = {
					event = "impound:ask:client:scuff"
				}
			},
			{
				id = 2,
				header = "Normal Impound $500",
				txt = "Send Vehicle To the Impound Lot",
				params = {
					event = "impound:ask:client:normal"
				}
			},
			{
				id = 3,
				header = "Police Impound $1500",
				txt = "Send Vehicle To the Police Impound",
				params = {
					event = "impound:ask:client:police"
				}
			},
			{
				id = 4,
				header = "Close Menu",
				txt = "Close Impound Menu",
				params = {
					event = ""
				}
			},
		...
		TriggerEvent('DoLongHudText', "You don't have the correct Job!", 2)
    end 
end)

function deleteVeh(ent, pImpoundType, Plate)
	SetVehicleHasBeenOwnedByPlayer(ent, true)
	NetworkRequestControlOfEntity(ent)
	local finished = exports["np-taskbar"]:taskBar(5000,"Impounding",false,true)
	if DoesEntityExist(ent) then
		if finished == 100 then
			remoteCalls.execute("np-garages:impound:request", pImpoundType, licensePlate)
			Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(ent))
			DeleteEntity(ent)
			DeleteVehicle(ent)
			SetEntityAsNoLongerNeeded(ent)
		else
			TriggerEvent("DoLongHudText", "You stopped!", 2)
		end
	else
		TriggerEvent("DoLongHudText", "There is no target vehicle to impound!", 2)
	end
end

RegisterNetEvent("impound:ask:client:police", function()
	playerped = PlayerPedId()
    coordA = GetEntityCoords(playerped, 1)
    coordB = GetOffsetFromEntityInWorldCoords(playerped, 0.0, 100.0, 0.0)
   	targetVehicle = getVehicleInDirection(coordA, coordB)
	licensePlate = GetVehicleNumberPlateText(targetVehicle)
	deleteVeh(targetVehicle, "Police Impound", licensePlate)
end)


RegisterNetEvent("impound:ask:client:normal", function()
	playerped = PlayerPedId()
    coordA = GetEntityCoords(playerped, 1)
    coordB = GetOffsetFromEntityInWorldCoords(playerped, 0.0, 100.0, 0.0)
   	targetVehicle = getVehicleInDirection(coordA, coordB)
	licensePlate = GetVehicleNumberPlateText(targetVehicle)
	deleteVeh(targetVehicle, "Normal Impound", licensePlate)
end)


RegisterNetEvent("impound:ask:client:scuff", function()
	playerped = PlayerPedId()
    coordA = GetEntityCoords(playerped, 1)
    coordB = GetOffsetFromEntityInWorldCoords(playerped, 0.0, 100.0, 0.0)
   	targetVehicle = getVehicleInDirection(coordA, coordB)
	licensePlate = GetVehicleNumberPlateText(targetVehicle)
	deleteVeh(targetVehicle, "scuff", licensePlate)
end)
...
AddEventHandler("attempt:duty:police", function()
	if not isInService then
		...
		TriggerEvent('DoLongHudText', 'You are already on duty!', 2)
	end
end)
...
AddEventHandler("serial:search", function()
    local serialS = exports["np-keyboard"]:KeyboardInput({
...
AddEventHandler("fired:success", function(cidsent)
	...
	if tonumber(cid) == tonumber(cidsent) then
		...
		TriggerEvent("np-jobmanager:playerBecameJob", "Unemployed", "Unemployed", false)
	end
end)
...
Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("evidence_spot", vector3(475.48, -1005.78, 26.27), 1.8, 2, {
		name="evidence_spot",
		heading=90,
		--debugPoly=true,
		minZ=25.27,
...
Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("SandyPDEvidence", vector3(1857.14, 3690.61, 29.82), 4.0, 3.2, {
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "evidence_spot" then
        ...
        if job == "police" then
            near_evidence = true
            TriggerEvent('np-textui:ShowUI', 'show', ("%s"):format("/evidence [number]"))
        end
    end
end)
...
RegisterNetEvent('taser:client:refill')
AddEventHandler('taser:client:refill', function()
    local finished = exports["np-taskbar"]:taskBar(3000, "Reloading Taser")
    if finished == 100 then
        taserCartsLeft = maxTaserCarts
        TriggerEvent("inventory:removeItem", "taserammo", 1)
    end
end)
        ...
        if taserCartsLeft <= 0 then
            ...
            if GetSelectedPedWeapon(ped) == taserModel then
                ...
                if IsControlJustPressed(0, 106) then
                    TriggerEvent("DoLongHudText", "You have no taser cartridges left. Reload more taser cartridges!", 2)
                end
            end
        end
...
local BoatGarageBlip = vector3(-806.04351806641, -1496.5289306641, 0.89005941152573)
...
Citizen.CreateThread(function()
    while true do
        ...
        if myJob == 'police' then
            havingPoliceJob = true
            if PDBoatGarage == nil or PDBoatGarage == 0 then
                PDBoatGarage = AddBlipForCoord(BoatGarageBlip)
                SetBlipSprite(PDBoatGarage, 68)
                SetBlipDisplay(PDBoatGarage, 4)
                SetBlipScale(PDBoatGarage, 0.7)
                SetBlipColour(PDBoatGarage, 3)
                SetBlipAsShortRange(PDBoatGarage, true)
                BeginTextCommandSetBlipName("STRING")
                AddTextComponentString("PD Boat Garage")
                EndTextCommandSetBlipName(PDBoatGarage)
            end    
        else
            havingPoliceJob = false
            if PDBoatGarage ~= nil or PDBoatGarage ~= 0 then
                RemoveBlip(PDBoatGarage)
                PDBoatGarage = 0
            end
        end
    end
