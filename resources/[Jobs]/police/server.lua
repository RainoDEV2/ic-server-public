...
AddEventHandler('police:jailGranted', function(args)
	...
	local target = exports["np-fw"]:getModule("Player"):GetUser(player)
	local character = target:getCurrentCharacter()
	local playerName = character.first_name .. ' ' .. character.last_name

	local pdunit = exports["np-fw"]:getModule("Player"):GetUser(src)
	...
	exports["np-fw"]:getModule("JobManager"):SetJob(target, "unemployed", true)
end)
...
AddEventHandler('police:SeizeCash', function(target)
	...
	local userz = exports["np-fw"]:getModule("Player"):GetUser(target)
...
AddEventHandler("police:bill:player", function(TargetID, amount)
	...
	if user ~= false then
		if fine > 0 then
			...
			exports["np-banking"]:UpdateSociety(fine, "police", "add")
		end
	end
end)
...
AddEventHandler("911", function(args)
	...
	local users = exports["np-fw"]:getModule("Player"):GetUsers()

	local emergencyPlayers = {}

	for k,v in pairs(users) do
		local user = exports["np-fw"]:getModule("Player"):GetUser(v)
		local job = user:getVar("job")
...
AddEventHandler("police:dnaAsk", function(t)
	local src = source
	local user = exports["np-fw"]:getModule("Player"):GetUser(t)
	local character = user:getCurrentCharacter()
...
AddEventHandler('police:grantHunting', function(t)
	...
	exports.ghmattimysql:execute('SELECT * FROM user_licenses WHERE `owner`= ? AND `type` = ? AND `status` = ?', {character.id, "Hunting", "0"}, function(data)
		if data[1] then
			...
			TriggerClientEvent("np-hunting:allowed", src, true)
			user:removeBank(250)
		else
...
AddEventHandler('police:getLicenses', function(t)
	...
	local user = exports["np-fw"]:getModule("Player"):GetUser(t)
	local char = user:getCurrentCharacter()
	exports.ghmattimysql:execute("SELECT * FROM user_licenses WHERE owner = ? AND type = ?", {char.id, "firearm"}, function(result)
		exports.ghmattimysql:execute("SELECT * FROM user_licenses WHERE owner = ? AND type = ?", {char.id, "Driver"}, function(result2)
			exports.ghmattimysql:execute("SELECT * FROM user_licenses WHERE owner = ? AND type = ?", {char.id, "Hunting"}, function(result3)
				if result[1] ~= nil or result2[1] ~= nil or result3[1] ~= nil then
					TriggerClientEvent("chatMessage", src, "License Center : ", 4, "Weapon License [".. result[1].status .."] | Drivers License [" .. result2[1].status .."] | Hunting License [".. result3[1].status .."]")
				end
			end)
		end)
	end)
...
AddEventHandler("serials:insert", function(srcID, serial)
	local user = exports["np-fw"]:getModule("Player"):GetUser(srcID)
...
AddEventHandler("doj:announce", function(srcID, args)
	local user = exports["np-fw"]:getModule("Player"):GetUser(srcID)
	local characterId = user:getCurrentCharacter().id
	exports.ghmattimysql:execute("SELECT `pass_type` FROM character_passes WHERE cid = @cid", {['cid'] = characterId}, function(result)
		if result[1] then
			if result[1].pass_type == "judge" then
...
RegisterServerEvent('police:dragAsk')
AddEventHandler('police:dragAsk', function(argument)
    local src = source
    local user = exports["np-fw"]:getModule("Player"):GetUser(src)
    TriggerClientEvent("dr:dragging", src)
    TriggerClientEvent("dr:drag", tonumber(argument), src)
end)