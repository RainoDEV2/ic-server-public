...
AddEventHandler("mech:tools", function(args)
    local shop = exports["np_handler"]:isPed("myjob")
    if exports["np-inventory"]:hasEnoughOfItem(args[1],tonumber(args[2]),false) then
...
function repairVeh(args)
    ...
    if job == 'tuner_shop' or job == 'auto_exotics' or job == 'harmony_autos' or job == 'hayes_autos' then 
        ...
        if amount < 10 then
            ...
            local finished = exports["np-taskbar"]:taskBar(amount*1000,"Repairing")
