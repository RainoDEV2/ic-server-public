...
local RolePriority = {
    ["794316196840341532"] = 500, -- Mortal
    ["794316196840341529"] = 90, --  Admin
    ["885686271907860480"] = 75, --  Mod
    ["826175255273668648"] = 80, --  iciest
    ["826175137941422080"] = 70, --  Diamond
    ["826175065795985508"] = 60, --  Gold
    ["826174980613603329"] = 50, --  Silver
    ["826174832890478603"] = 30, --  Bronze
    ["794316196828545092"] = 15, --  LSPD
    ["794316196828545087"] = 10, --  EMS
}
...
_Queue.JoinCbs = {}
_Queue.TempPriority = {}
_Queue.JoinDelay = GetGameTimer() + 30000
...
exports("ConnectHit", function()
    playerConnect(name, setKickReason, deferrals)
end)
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    deferrals.defer()
    Wait(50)
    deferrals.update("Joining iciest 🚀")
    Wait(1000)
    ...
    elseif isBanned then
        ...
        Wait(0)
        deferrals.done(message)
    elseif steamid == nil or steamid == "Unidentified" then
        local message = "You do not appear to have your Steam linked"
        Wait(0)
        deferrals.done(message)
    elseif discord == nil or discord == "Unidentified" and not canBypass[steamid] then
        local message = "You do not appear to have your Discord linked"
        Wait(0)
        deferrals.done(message)
    elseif canBypass[steamid] then
        local ids = Queue:GetIds(src)
        local name = GetPlayerName(src)
        local connectTime = os_time()
        
        Queue:AddPriority(ids[1], 10000)
        ...
        local function done(msg, _deferrals)
            ...
            --exports['discordroles']:isRolePresent(discord:sub(9), {'794316196816093222'} --[[ can be a table or just a string. ]], function(hasRole, roles)
            local hasRole = true
            local roles = {'794316196816093222'}
            print(hasRole, roles)
                if not roles then
                    connecting = false
                    local message = "You are not in our discord"
                    Wait(0)
                    deferrals.done(message)
                elseif not hasRole then
                    connecting = false
                    deferrals.defer()
                    Wait(0)
                    deferrals.update("Not In Our Discord... discord.gg/iciest .")
                    Wait(0)
                    local message = "Please join the discord to play on the server: Discord: discord.gg/iciest "
                    Wait(0)
                    deferrals.done(message)
                else
                    local ids = Queue:GetIds(src)
                    ...
                    for k,v in pairs(roles) do
                        prio = prio + (RolePriority[v] or 0)
                        print(hasRole, roles)
                        print('roles: '..json.encode(roles))
                    end

                    Queue:AddPriority(ids[1], prio)
...
function NotWhitelisted(deferrals)
